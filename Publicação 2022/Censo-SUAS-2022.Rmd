...
gerh_2019 <- read_excel("../Censo SUAS 2019/GestÆo Estadual/Censo_SUAS_2019_RH_GE_divulgaÆo.xlsx")
...
gm_2019 <- read_excel("../Censo SUAS 2018/Censo_SUAS_2019_Gestão_Municipal_dados_gerais_divulgacao.xlsx")
...
gm_rh_2021 <- read_excel("../Censo SUAS 2021/3 - Gestao Municipal/Censo_SUAS_Gestão_Municipal_RH.xlsx")
...
A quantidade de trabalhadoras (es) nas Secretarias Estaduais de Assistência Social em 2012 era de 16.742 profissionais, considerando trabalhadores lotados na sede do órgão gestor e nas unidades públicas que ofertam serviços socioassistenciais. Esse quantitativo vem caindo, em 2022 passou a ter 4.275, redução de 12.467 trabalhadoras (es) neste período de 10 anos. 



```{r qtd-trab-uf, fig.cap ="Evolução da quantidade de trabalhadores (as) nas Secretarias Estaduais de Assistência Social - Brasil e grandes regiões, 2012, 2017 e 2022", fig.footer = "Fonte: MDS, Censo SUAS."}
...
if(gera.graficos.office) grafico %>% graph2office(file="graficos", append = TRUE)
```
<!--fazer grafico para gestão municipal-->.

<!-- as Gestões Municipais informaram ter 239.815 trabalhadoras e trabalhadores exercendo funções/atividades ligadas à assistência social (inclusive aqueles lotados nas unidades públicas), o menor número desde 2012. Observando a série histórica é possível verificar que o número máximo de trabalhadores foi registrado no ano de 2014, após sucessivos crescimentos desde 2010. Desde de 2015 esse número se reduziu, registrando em 2016 cerca de 17.000 trabalhadores a menos que em 2014 (Gráfico 60). Gráfico 60: Evolução da quantidade de trabalhadores nas Secretarias Municipais de Assistência Social - Brasil, 2010 a 2016 Fonte: MDS, Censo SUAS.-->.

\footnote{os dados totais de trabalhadores da gestão municipal para o ano de 2017 não foram possíveis de serem quantificados em decorrência de alterações na modalidade de respostas na base de dados}

<!--Valdson, na pratica dos dados de 2022 estão ok, mas na formula tem regi~çoes que não estão somando -->.

```{r qtd_trab_munic, fig.cap ="Evolução da quantidade de trabalhadores nas Secretarias Municipais de Assistência Social - Brasil, 2012 a 2022" }

grafico <- f_soma_regiao_ano(gm_2012, q38e_4, codigo_ibge, "2012") %>%
  #bind_rows(f_soma_regiao_ano(gm_2017, d28.5.4, IBGE, "2017")) %>%
  bind_rows(f_quantitativo_regiao_ano(gm_2022, IBGE, "2022")) %>%
  f_grafico_col_group_numero(Região, n, Ano, 1)
...
```{r uf_trab_vin, fig.cap ="Percentual de trabalhadores nas Secretarias Estaduais de Assistência Social, segundo tipo de vínculo - Brasil, 2010 a 2022", fig.height = 9.2 }
...
<!--Valdson, também não consgeui gerar este gráfico, entretanto os codigos das questões estão certinhas, deve aer alguma outra coisa que não estopu conseguindo -->.

```{r munic_trab_vin, fig.cap ="Percentual de trabalhadores nas Secretarias Municipais de Assistência Social, segundo tipo de vínculo - Brasil, 2012 a 2022", eval=FALSE}
...
grafico <- f_nomex_somay_nomegrupo(gm_rh_2012, "2012", q37a_4, "Estatutários") %>%
  bind_rows(f_nomex_somay_nomegrupo(gm_rh_2012, "2012", q37b_4, "Celetistas")) %>%
  bind_rows(f_nomex_somay_nomegrupo(gm_rh_2012, "2012", q37c_4, "Somente Comissionados")) %>%
  bind_rows(f_nomex_somay_nomegrupo(gm_rh_2012, "2012", q37d_4, "Outros Vínculos")) %>%
  ...
  # bind_rows(f_nomex_grupos_quantitativo(gerh_2018, "2018", q_37_11)) %>%
   ...
   #bind_rows(f_nomex_grupos_quantitativo(gmrh_2022, "2022", `nu_trabalhador`)) %>%
...
<!--fazer para as gestoes municipais-->.

```{r munic_trab_form, fig.cap =" Percentual de trabalhadores nas Secretarias municipais de Assistência Social, segundo escolaridade e profissão - Brasil, 2012 a 2022",eval=FALSE} 

grafico <- f_nomex_somay_nomegrupo(gm_rh_2012, "2012", q37e_1, "Nível Fundamental") %>%
  bind_rows(f_nomex_somay_nomegrupo(gm_rh_2012, "2012", q37e_2, "Nível Médio")) %>%
  bind_rows(f_nomex_somay_nomegrupo(gm_rh_2012, "2012", q37e_3, "Nível Superior")) %>%
...
```{r trab-unidades, fig.cap =" Evolução da quantidade de trabalhadores dos CRAS, CREAS e Centro POP - Brasil; 2012, 2017 e 2022" }
