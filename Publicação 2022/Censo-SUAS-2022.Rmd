...
#cedia_2022 <- read_csv2("../Censo SUAS 2022/6 - CENTRO DIA/Censo_SUAS_2022_Centro_Dia_dados_gerais.csv")
...
```{r municipais-constituicao-subdivisoes, fig.cap = "Distribuição dos órgãos gestores municipais segundo constituição/formalização de subdivisões administrativas - Brasil, 2017", fig.footer = "Fonte: MDS, Censo SUAS."}
...
**grafico municípios segundo lei municipal*
...
```{r municipios-atualizacao-PMAS, fig.cap = "Distribuição de municípios segundo ano de atualização do Plano Municipal de Assistência Social (PMAS), por grandes regiões - Brasil, 2017", fig.footer = "Fonte: MDS, Censo SUAS."}
...
Foi observada redução no percentual de estados que realizavam cofinanciamento por convênio: o número passou de 15,4% (4) em 2013 para 3,8% (1) em 2016 (\Cref{fig:estados-cofinanciamento-municipios}).
...
```{r estados-cofinanciamento-municipios, fig.cap = "Percentual de estados segundo realização de cofinanciamento aos municípios - Brasil, 2010 a 2017", fig.footer = "Fonte: MDS, Censo SUAS."}
...
grafico <- ge_cofinanciamento(ge_2010, q24, "2010") %>%
  ...
  bind_rows(ge_cofinanciamento(ge_2012, q64_0, "2012")) %>%
...
tabela <- ge_destinacao_recursos(ge_2013, q75a, "2013", "1") %>%
...
Entre 2013 e 2016 o percentual de estados que prestavam assessoramento técnico a distância aumentou, passando de 61,5% em 2013 para 92,3% em 2016. O percentual de estados que prestava capacitações à distância, por sua vez, diminuiu, passando de 42,3% em 2013 para 26,9% em 2016 (\Cref{fig:estados-formas-apoio}).

```{r estados-formas-apoio, fig.cap = "Percentual de estados segundo formas de apoio técnico aos municípios - Brasil, 2013 a 2017", fig.footer = "Fonte: MDS, Censo SUAS."}
...
tabela <- ge_apoio_tecnico(ge_2013, q83a, "2013", "1") %>%
...
Entre os anos de 2015 e 2016, é possível identificar redução nos percentuais de Secretarias exclusivas em todas as regiões, com exceção da região Norte (\Cref{fig:secretarias-municipais-exclusivas}).
...
```{r municipios-recebimento-recursos, fig.cap = "Distribuição de municípios por recebimento de recursos estaduais para o cofinanciamento da Assistência Social e destinação dos recursos recebidos - Brasil, 2017", fig.footer = "Fonte: MDS, Censo SUAS."}
...
```{r municipios-receberam-visitas, fig.cap = "Percentual de municípios que receberam visitas de técnicos da Secretaria Estadual de Assistência Social (SEAS) - Brasil, 2013 a 2017", fig.footer = "Fonte: MDS, Censo SUAS."}
...
A modalidade na qual foi observado o menor percentual foi a de capacitações à distância, da qual participaram 29,7% dos municípios. Esse tipo de apoio ocorreu em 53,9% dos municípios da região Sudeste e em 14,3% dos municípios da região Centro-Oeste (\Cref{fig:municipios-formas-apoio}).
...
# Equipamentos, Serviços e Benefícios
...
O quantitativo de CRAS dobrou entre 2007 e 2022, passando de 4.195 unidades para 8.557. Desde 2007, 4.361 novas unidades de CRAS foram criadas nos territórios municipais - crescimento. Abaixo (\Cref{fig:quantitativo-CRAS}) evolução por grandes regiões de desenvolvimento.  


```{r quantitativo-CRAS, fig.cap = "Evolução do quantitativo de CRAS por Município - Brasil - Regiões, 2007 - 2012 - 2017 - 2022", fig.footer = "Fonte: MDS, Censo SUAS."}

grafico <- 
  #(f_quantitativo_regiao_ano(cras_2007, Gran_reg, "2007") %>%
  ...
  f_quantitativo_regiao_ano(cras_2012, Região, "2012") %>% 
  ...
  #bind_rows(f_quantitativo_regiao_ano(cras_2022, Região, "2022")) %>%
...
```{r CREAS-sit,eval=FALSE, fig.cap = "Evolução dos CREAS segundo situação do imóvel – Brasil,  2012 - 2017 - 2022",fig.footer = "Fonte: MDS, Censo SUAS."}

f_creas_imovel = function(df, q, ano){
  q <- enquo(q)
  ano <- quo_name(ano)
...
#grafico <- f_cras_imovel(cras_2007, v15, "2007") %>%
  #bind_rows(f_cras_imovel(cras_2008, situacao_imovel, "2008")) %>% 
  #bind_rows(f_cras_imovel(cras_2009, q5_situac_imovel, "2009")) %>% 
  #bind_rows(f_cras_imovel(cras_2010, q5, "2010")) %>% 
  #bind_rows(f_cras_imovel(cras_2011, q5, "2011")) %>%
  grafico <- f_creas_imovel(creas_2012, q15, "2012") %>%
  bind_rows(f_cras_imovel(cras_2012, q5, "2012")) %>%
  #bind_rows(f_cras_imovel(cras_2013, q5, "2013")) %>%
  #bind_rows(f_cras_imovel(cras_2014, q4, "2014")) %>%
  #bind_rows(f_cras_imovel(cras_2015, q4, "2015")) %>%
  #bind_rows(f_cras_imovel(cras_2016, q4, "2016")) %>%
  bind_rows(f_cras_imovel(creas_2017, q4, "2017")) %>%
  bind_rows(f_cras_imovel(creas_2022, q3, "2022")) %>%
...
#  bind_rows(equip_acessibilidade(cras_2011, q10d, "2011", "4")) %>% 
   bind_rows(equip_acessibilidade(cras_2012, q10a, "2012", "1")) %>%
...
```{r CRAS-internet, fig.cap = "Percentual  de CRAS com computador com acesso a internet, 2012 - 2017 - 2022", fig.footer = "Fonte: MDS, Censo SUAS.", eval=FALSE}
...
#  bind_rows(f_cras_imovel(cras_2015, q4, "2015")) %>%
#  bind_rows(f_cras_imovel(cras_2016, q4, "2016")) %>%
#  bind_rows(f_cras_imovel(cras_2017, q4, "2017")) %>%
#  bind_rows(f_cras_imovel(cras_2022, q11_1, "2022")) %>%
...
```{r CRAS-internet-percentual, fig.cap = "Evolução percentual de CRAS com computadores com acesso à internet – Brasil, 2007 a 2017", fig.footer = "Fonte: MDS, Censo SUAS."}
...
```{r centro-conv-natureza, fig.cap = "Quantitativo de Centros de Convivência por Natureza da Unidade - BRASIL, 2014 - 2018 - 2022", fig.footer = "Fonte: MDS, Censo SUAS."}
...
```{r municipios-CREAS, fig.cap = "Evolução de CREAS por número de municípios, segundo grandes regiões - Brasil, 2012 - 2017 - 2022,", fig.footer = "Fonte: MDS, Censo SUAS."}
...
```{r CREAS-municipios_2, fig.cap = "Evolução de CREAS Regionais (res. 21/2023)", fig.footer = "Fonte: MDS, Censo SUAS."}

```


```{r creas-situacao, fig.cap = "Evolução dos CREAS segundo situação do imóvel – Brasil, 2012 - 2017 - 2022",fig.footer = "Fonte: MDS, Censo SUAS."}
...
#grafico <- f_creas_imovel(creas_2012, q5, "2012") %>%
  #bind_rows(f_creas_imovel(creas_2008, situacao_imovel, "2008")) %>% 
  #bind_rows(f_creas_imovel(creas_2009, q5_situac_imovel, "2009")) %>% 
  #bind_rows(f_creas_imovel(creas_2010, q5, "2010")) %>% 
  #bind_rows(f_creas_imovel(creas_2011, q5, "2011")) %>%
grafico <- f_creas_imovel(creas_2012, q5, "2012") %>%
  #bind_rows(f_creas_imovel(creas_2013, q5, "2013")) %>%
  #bind_rows(f_creas_imovel(creas_2014, q4, "2014")) %>%
  #bind_rows(f_creas_imovel(creas_2015, q4, "2015")) %>%
  #bind_rows(f_creas_imovel(creas_2016, q4, "2016")) %>%
  bind_rows(f_creas_imovel(creas_2017, q4, "2017")) %>%
  bind_rows(f_creas_imovel(creas_2022, q3, "2022")) %>%
...
tabela <- 
  # equip_acessibilidade(creas_2010, q9_1, "2010", "1") %>%
  ...
  bind_rows(equip_acessibilidade(creas_2012, q10a, "2012", "1")) %>%
...
```{r CREAS-Abordagem_Social, fig.cap = "Quantidade de CREAS que realizam o Serviço Especializado em Abordagem Social, 2015 - 2023", fig.footer = "Fonte: MDS, Censo SUAS."}
...
```{r CentroPop-internet-percentual, fig.cap = "Percentual de Computadores nos Centro Pop com acesso a internet, 2012 a 2022", fig.footer = "Fonte: MDS, Censo SUAS."}
...
```{r CentroDia-evol, fig.cap = "Evolução de CENTRO DIA  - 2015 a 2022", fig.footer = "Fonte: MDS, Censo SUAS."}
...
```{r CentroDia-Natureza, fig.cap = "Quantitativo de Centros Dia por Natureza da Unidade - BRASIL, 2015 a 2022", fig.footer = "Fonte: MDS, Censo SUAS."}
...
```{r centro-dia_acessibilidade, fig.cap = "Evolução percentual dos Centros-Dia segundo condições de acessibilidade de acordo com a Norma da ABNT - Brasil, 2015 e 2022", fig.footer = "Fonte: MDS, Censo SUAS."}
...
```{r Povostradicionais, fig.cap = "Número de CRAS, CREAS que atendem povos e comunidades tradicionais - Brasil, 2014  - 2022",fig.footer = "Fonte: MDS, Censo SUAS."}
...
```{r BE, fig.cap = "Percentual de Municípios que concedem Benefícios Eventuais - 2014_2022",fig.footer = "Fonte: MDS, Censo SUAS."}


```


```{r munic-BE, fig.cap = "Percentual de Municípios que concedem Benefícios Eventuais - 2022", fig.footer = "Fonte: MDS, Censo SUAS."}
