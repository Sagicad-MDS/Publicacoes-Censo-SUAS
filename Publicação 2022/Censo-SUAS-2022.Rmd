...
-   **Unidades, Serviços e Benefícios da Assistência Social:**
...
são os mais altos percentuais de informalidade e não constituição. Destaca-se também a proteção social especial, area recomendada existir independe da presença de undiades de CREAS. 
...
possuem Núcleo de Educação Permante (\Cref{fig:NUEP}). 
...
13,9% (769) dos municípios possuem este plano. Esse percentual, como pode ser 
observado (\Cref{fig:PNEP_Munic})teve evolução residual últimos anos. 
...
\footnote{o formulário do posto do cadastro único foi criado em 2020, assim a maioria das informações disponíveis terão referencia esta data}.
...
do acesso a Serviços, Programas, benefícios, identificação das famílias e 
territórios vulnerabilizados, bem como ações intersetoriais.
...
Dados do Censo SUAS mostram uma evolução de locais de cadastramento, sobretudo a 
partir de 2017. 
...
61,0% encontram-se na sede da Secretaria de Assistência Social e 24,3% em
...
levantamento de famílias para atualização e inclusão cadastral são a mais 
...
Em relação as informações de unidades de Postos do Cadasto Único e o atendimento de Grupos Tradicionais e Especificos - GPTEs, destaca-se através (\Cref{gptes_cadunico}) o percertual de unidades de postos do cadastro único que informam atender no período de 2020 a 2022. 

```{r gptes_cadunico, fig.cap = "Unidades que realiza cadastramento de famílias pertencentes aos seguintes Grupos Tradicionais e Específicos - GPTEs - Brasil, 2020 - 2021 - 2022", fig.footer = "Fonte: MDS, Censo SUAS"}

gptes_cadunico  = function(df, q, ano){
  q <- enquo(q)
  ano <- quo_name(ano)
...
grafico <- gptes_cadunico(posto_cadunico_2020,q20_1,"2020") %>%
          bind_rows(gptes_cadunico(posto_cadunico_2021,q20_1,"2021"))%>%
          bind_rows(gptes_cadunico(posto_cadunico_2022,q20_1, "2022"))%>%
        #gptes_cadunico(coord_cadunico(posto_cadunico_2022, q20_3, "2022"))%>%
        #gptes_cadunico(coord_cadunico(posto_cadunico_2022, q20_3, "2022"))%>%
        #gptes_cadunico(coord_cadunico(posto_cadunico_2022, q20_3, "2022"))%>%
...
Em relação as ações de complementariedade com a rede socioassistencial, destaca-se que a maioria destas undiades de 
cadastramento realiza encaminahmentos para rede socioassistencial de CRAS, CREAS, Centro POp entre outros.

```{r redesuas, fig.cap = "Percentual de Postos do Cadastro Único que realizam encaminhamentos para rede - Brasil, 2020 - 2021 - 2022", fig.footer = "Fonte: MDS, Censo SUAS"}

rede_suas  = function(df, q, ano){
  q <- enquo(q)
  ano <- quo_name(ano)
...
grafico <- rede_suas(posto_cadunico_2020, q32_1, "2020") %>%
        bind_rows(rede_suas(posto_cadunico_2021, q32_1, "2021"))%>%
        bind_rows(rede_suas(posto_cadunico_2022, q32_1, "2022"))%>%
...
O grafico abaixo (\Cref{fig:igdbolsa}) sinaliza sobre a existência da participação das unidades do Cadastro único no planejamento 
dos recursos recebidos no âmbito do IGDPBF (Índice de Gestão Descentralizada da Gestão do Cadastro Único e Bolsa Família).
...
a respostas do dormulário dos potos de cadastramento em ambito dos municípios.
...
<!-- colocar item abaixo na gestão do trabalho.}). -->
...
reafirmadas por Lei, Normas, decretos, pactos. Assim, faz-se essencial padrões de gestão e 
comandos únicos em todos os entes federados. Os dados históricos sinalizam avanços e desafios 
para gestão do SUAS.

Os órgãos gestores da assistência social, tanto estaduais quanto
municipais, são estruturas fundamentais para a execução deste política. O significado
do direito caminham junto com o planejamento, ofertas continuadas, padrões de respostas, comandos unicos etc.

Assim, para consolidação desta política que faz-se universal nos territórios,é fundamental
em todos entes federados estruturas administrativas indicadas nos pactos, Lei compatível com LOAS, 
conforme deliberação das conferencias nacionais, comando único do gestor da pasta entre outros.  

Os dados sinalizam avanços, a respeito das subdivisões administrativas que apresentavam maior percentual de
formalização nos órgãos gestores estaduais em 88,5% dos estados. As que apresentavam menores
percentuais de formalização nos órgãos gestores estaduais eram as de
Regulação do SUAS (38,5%) e de Gestão do Trabalho (57,7%). Nos municípios, destaca-se forte presença de formalização
das gestões do Cadastro Único e Bolsa Família, bem como a proteção social básica, com respectivamente 80,68% e 77,90% respectivamente.
...
Bipartite e Tripartite. Sobre o repasse, destacam-se o avanço ao longo dos anos nesta forma repasse aliado ao cofinanciamento, sendo mais presente
o cofinancimento a Proteção Social Especial.

Destaca-se também um tópico novo e de importancia para gestão de informação do SUAS. O cadatsro único é uma ferramenta
que visa potencializar as funções de proteção socia, vigilancia socioassitencial e defesa de direitos, Para isso, os dados 
promovem acesso a serviços e progrmas, identificação das fmaílias e leitura dos territórios, bem como parcerias e ações intersetoriais.
...
ceas O Conselho é a Instância de Controle Social do Programa Bolsa Família (Resolução CNAS 18/2013 e Resolução CNAS 15/2014) ...

```{r ceas_igd, fig.cap = "igd"}
...
```{r ceas_icspbf, fig.cap = "Conselhos Estaduais de Assistência Social que são instancia de controle Social do PBF"}

ceas_icspbf  = function(df, q, ano){
  q <- enquo(q)
  ano <- quo_name(ano)
...
grafico <- ceas_icspbf(cest_2012, q39, "2012") %>%
   #bind_rows(ceas_icspbf(cest_2013, q40, "2013")) %>%
   bind_rows(ceas_icspbf(cest_2014, q41, "2014")) %>%
   bind_rows(ceas_icspbf(cest_2015, q39, "2015")) %>%
   bind_rows(ceas_icspbf(cest_2016, q32, "2016")) %>%
   bind_rows(ceas_icspbf(cest_2017, q47, "2017")) %>%
   bind_rows(ceas_icspbf(cest_2018, q_43, "2018")) %>%
   bind_rows(ceas_icspbf(cest_2019, q39, "2019")) %>%
   bind_rows(ceas_icspbf(cest_2020, q40, "2020")) %>%
   bind_rows(ceas_icspbf(cest_2021, q40, "2021")) %>%
   bind_rows(ceas_icspbf(cest_2022, q40, "2022")) %>%
...
grafico <- cmas_pmas(cmun_2012, q35_1, "2012") %>%
  bind_rows(cmas_pmas(cmun_2013, q40, "2013")) %>%
  bind_rows(cmas_pmas(cmun_2014, q41, "2014")) %>%
  bind_rows(cmas_pmas(cmun_2015, q39, "2015")) %>%
  bind_rows(cmas_pmas(cmun_2016, q32, "2016")) %>%
  bind_rows(cmas_pmas(cmun_2017, q47, "2017")) %>%
  bind_rows(cmas_pmas(cmun_2018, q_43, "2018")) %>%
  bind_rows(cmas_pmas(cmun_2019, q39, "2019")) %>%
  bind_rows(cmas_pmas(cmun_2020, q40, "2020")) %>%
  bind_rows(cmas_pmas(cmun_2021, q40, "2021")) %>%
  bind_rows(cmas_pmas(cmun_2022, q40, "2022")) %>%
