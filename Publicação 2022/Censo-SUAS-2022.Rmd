...
Sobre a distribuição destes locais de Cadastro Único, destaca-se que em 2022, 31% (2.892) são unidades exclusivas de Cadastro Único e 68% estão em unidades da rede socioassistencial \footnote{CRAS, CREAS e CENTRO POP} conforme pode ser observado na
...
dúvidas sobre serviços, programas entre outros são realizadas pela grande maioria das
unidades do cadastro único (96,82%). Já as ações de agendamento para
atendimento são realizadas por 64,35% das unidades, com redução ao longo
dos anos.
...
```{r redesuas, fig.cap = "Situações mais frequentes de entrevistas domiciliares no Cadastro Único - Brasil, 2020, 2021 e 2022", fig.footer = "Fonte: MDS, Censo SUAS."}
...
Abaixo o gráfico trás informações da quantidade de conselhos municipais,
...
```{r qtd-ceas, fig.cap = "Quantidade de Conselhos Estaduai de Assistência Social que preencheram Censo SUAS"}
...
if(gera.graficos.office) grafico %>% graph2office(file="graficos", append = TRUE)
```
Em realção aos Conselhos estaduais, atualmente 100% informam possuir sede especifica para funcionamento do controle social no SUAS conforme pdoe ser observado abaixo (\Cref{fig:ceas_sede}) 
...
```{r cas_se, fig.cap = "Percentual de Conselhos Municipais que possuem secretários (as) executivos exclusivamente no Conselho"}
...
if(gera.graficos.office) grafico %>% graph2office(file="graficos", append = TRUE)
```
```{r cmas_se, fig.cap = "Percentual de Conselhos Estaduais que possuem secretários (as) executivos exclusivamente no Conselho"}
...
grafico <- f_sim_nao(cest_2012, q13, "2012") %>%
  ...
  #2012, 2017 é perguntado apenas se existe SE, não pergunta de forma exclusiva
...
```{r qtdcmas_reuniao, fig.cap = "Proporção conselhos municipais quando a realização de reuniões plenárias do Conselho (ordinárias e extraordinárias)"}
...
#-------------------
# Questionários 2022:
  select("Quantidade de reuniões - ordinárias e extraordinárias"=d25) %>% 
#-------------------
  gather("Categoria","Nível") %>% 
  mutate(Categoria=str_wrap(Categoria,width=22)) %>% 
...
#-------------------
# Censo 2022:
  mutate(Nível = case_when(Nível <= 8~"De 0 a 8 reuniões",
                           Nível <= 9~"De 9 a 16 reuniões",
                           Nível <= 17~"De 17 a 20 reuniões",
                           Nível <= 21~"De 21 a 30 reuniões",
                           Nível <= 31~"De 31 a 40 reuniões",
                           TRUE ~ as.character(Nível))) %>% 
  mutate(Nível = factor(Nível, levels = rev(c("De 0 a 8 reuniões","De 9 a 16 reuniões", "De 17 a 20 reuniões", "De 21 a 30 reuniões", "De 31 a 40 reuniões")))) %>% 
#-------------------
  group_by(Categoria, Nível) %>% 
  summarise(n=n()) %>% 
  group_by(Categoria) %>% 
