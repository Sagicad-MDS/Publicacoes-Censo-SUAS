...
posto_cadunico_2021 <- read_excel("../Censo SUAS 2020/5 - Postos do cadunico/Censo_SUAS_2020_Postos_Cadastro_Único_Dados_Gerais.xlsx", sheet = 1)
...
posto_cadunicorh_2021 <- read_excel("../Censo SUAS 2022/8 - POSTO DE CADASTRAMENTO/Censo _SUAS_ 2022_Posto_cadastramentoo_RH.xlsx", sheet = 1)
...
#cest_2019 <- read_excel("../Censo SUAS 2019/", sheet = 1)

#cestrh_2019 <- read_excel("../Censo SUAS 2019/", sheet = 1)
...
#cmun_2019 <- read_excel("../Censo SUAS 2019/  ", sheet = 1)

#cmunrh_2019 <- read_excel("../", sheet = 1)
...
```{r estados-caracteristica-secretaria-2012, fig.cap = "Percentual de Secretarias Estaduais de acordo com composição administrativa relacionada a outras políticas - Brasil, 2022", fig.footer = "Fonte: MDS, Censo SUAS, eval=FALSE"}
...
Observa-se que meta chega em 2022 com 22 (85%) estados com as subdivisões administrativas das áreas de Proteção Social Básica, Proteção Social Especial e Gestão do SUAS constituídas formalmente. São 4 areas que não possuem 100% contituídas (formal e informal) são elas: Gestão do SUAS, Gestão do trabalho, Serviços e Beneficios e Regulação do SUAS. Esta última apesenta menor percentual de formalização, sendo 38,5% subdivisões constituídas formalmente conforme \Cref{fig:estados-constituicao-subdivisoes}.
...
Entratando, 11 estados que não possuem Lei do SUAS. Esse número estão presentes nas seguintes regiões do Brasil: 4 - Nordeste, 3 - Norte, 3 - Região Sul e 1 Região Sudeste\footnote{ siglas dos Estado que não possuem Lei do SUAS - censo suas 2022: RR, PA, TO, PI, RN, SE, BA, SP, PR, SC e RS}.
...
```{r PEAS, fig.cap = "Percentual de Estados que não atualizaram Plano Estadual de Assistência Social em 2022 - Brasil", fig.footer = "Fonte: MDS, Censo SUAS"}
...
ord_despesa = function(df, q, ano){
  ...
  df %>%
    select(!! ano := !! q) %>% 
    gather("Categoria","Nível") %>% 
    mutate(Nível = case_when(Nível == "Secretário(a) Estadual de Assistência Social"~"O(a) Secretário(a) Estadual de Assistência Social",
                             TRUE ~ Nível)) %>% 
    filter(!is.na(Nível)) %>% 
...
grafico <- ord_despesa(feas_2018, q_2, "2018") %>%
...
```{r munic_ord_despesa, fig.cap = "Percentual de municípios quanto a ordenação de despesa do fundo estadual - Brasil, 2022", fig.footer = "Fonte: MDS, Censo SUAS."}
...
grafico <- ord_despesa(fmas_2018, q_2, "2018") %>%
...
```{r conc_munic, fig.cap = "Percentual de municípios que realizaram concursos para trabalahdores nível superior ou médio do SUAS - Brasil, 2020 - 2021 - 2022", fig.footer = "Fonte: MDS, Censo SUAS"}
...
```{r conc_estado, fig.cap = "Percentual de estados que realizaram concursos para trabalhadores nível superior do SUAS - Brasil, 2020 - 2021 - 2022", fig.footer = "Fonte: MDS, Censo SUAS"}
...
O Cadastro Único para Programas Sociais do Governo Federal estabelecido através do art. 6º-F da Lei nº 8.742, de 7 de dezembro de 1993 (Lei Orgânica de Assistência Social – LOAS), tem a finalidade de coletar, sistematizar e disseminar informações que permitem a identificação e caracterização das condições socioeconômicas das famílias em situação de vulnerabilidade social, em particular as famílias de baixa de renda. 

O objetivo é conhecer, incluir e aprimorar as políticas sociais. Assim, esses dados subsidiam a realização de diagnósticos, ações de planejamento, busca ativa, monitoramento e avaliação. Sua gestão é interfederativa e descentralizada. Os locais de cadastramento são efetuados pelos municípios no qual podem ser encontrados em unidades exclusivas ou na rede de atendimento socioassistencial. 

Dados do Censo SUAS 2022 destaca que xx% (2.892) são unidades exclusivas do Cadastro Único e xx estão em unidades da rede socioassistencial \footnote{os dados dos locais do Cadastro Único na rede socioassistencial CRAS, CREAS E CENTRO POP serão abordadas nos respectivos temas}.




<!-- colocar tabela com anos e quantidade de locais para cadasatro unico   -->
...
```{r postos-cadunico, fig.cap = "Evolução de Postos do Cadastro Único - 2020 - 2021 - 2022", fig.footer = "Fonte: MDS, Censo SUAS."}
...
Sobre as unidades exclusivas do Cadastro Único a sua maioria \footnote{mais de 85%} se encontram na sede da secretaria de Assistência Social ou em postos exclusivos. Os demais percentuais são encontrados em outra unidade administrativa, em um serviço integrado, OSC´s, conselho ou escola.  
...
<!-- não sei porque não centro oeste ficou duplamente registrado   --> 
...
grafico <- f_maior0_regiao_ano(posto_cadunico_2020, q10_2, Regiao, "2020") %>% 
...
<!-- não consegui gerar os demais e queria colocar em número em vez de percentual - figura acima -->
...
```{r equip-cadunico-centropop, fig.cap = " Percentual de Centro Pop com locais para CadÚnico e disposição da equipe - 2018 - 2022", fig.footer = "Fonte: MDS, Censo SUAS",eval=FALSE}
...
grafico <- cmas_sede(cmun_2012, q8, "2012") %>%
  ...
  #bind_rows(cmas_sede(cmun_2014, q8, "2014")) %>%
  #bind_rows(cmas_sede(cmun_2015, q8, "2015")) %>%
  #bind_rows(cmas_sede(cmun_2016, q8, "2016")) %>%
  bind_rows(cmas_sede(cmun_2017, q7, "2017")) %>%
  #bind_rows(cmas_sede(cmun_2018, q8, "2018")) %>%
  #bind_rows(cmas_sede(cmun_, q8, "2019")) %>%
  #bind_rows(cmas_sede(cmun_2020, q8, "2018")) %>%
  #bind_rows(cmas_sede(cmun_2021, q8, "2018")) %>%
...
```{r Ceas_sede, fig.cap = "Percentual de Conselhos Estaduais que possuem local/sede específico para funcionamento - Brasil, 2012 - 2017 - 2022"}
...
grafico <- ceas_sede(cest_2012, q8, "2012") %>%
  ...
  #bind_rows(ceas_sede(cest_2014, q8, "2014")) %>%
  #bind_rows(ceas_sede(cest_2015, q8, "2015")) %>%
  #bind_rows(ceas_sede(cest_2016, q8, "2016")) %>%
   bind_rows(ceas_sede(cest_2017, q7, "2017")) %>%
  #bind_rows(ceas_sede(cest_2018, q8, "2018")) %>%
  #bind_rows(ceas_sede(cest_2019, "2019")) %>%
  #bind_rows(ceas_sede(cest_2020, q8, "2018")) %>%
  #bind_rows(ceas_sede(cest_2021, q8, "2018")) %>%
...
```{r Cmas_se, fig.cap = "Percentual de Conselhos municipais com secretarias executivas - Brasil, 2012 - 2017 - 2022"}
...
grafico <- cmas_se(cmun_2012, q13, "2012") %>%
  ...
  #bind_rows(cmas_se(cmun_2014, q13, "2014")) %>%
  #bind_rows(cmas_se(cmun_2015, q13, "2015")) %>%
  #bind_rows(cmas_se(cmun_2016, q13, "2016")) %>%
  bind_rows(cmas_sede(cmun_2017, q13, "2017")) %>%
  #bind_rows(cmas_se(cmun_2018, q13, "2018")) %>%
  #bind_rows(cmas_se(cmun_, q8, "2019")) %>%
...
grafico <- cmas_pmas(cmun_2012, q35_1, "2012") %>%
  #bind_rows(cmas_pmas(cmun_2013, q, "2013")) %>%
  #bind_rows(cmas_pmas(cmun_2014, q, "2014")) %>%
  #bind_rows(cmas_pmas(cmun_2015, q, "2015")) %>%
  #bind_rows(cmas_pmas(cmun_2016, q, "2016")) %>%
  bind_rows(cmas_pmas(cmun_2017, q38, "2017")) %>%
  #bind_rows(cmas_pmas(cmun_2018, q, "2018")) %>%
  #bind_rows(cmas_pmas(cmun_, q, "2019")) %>%
  #bind_rows(cmas_pmas(cmun_2020, q, "2020")) %>%
  #bind_rows(cmas_pmas(cmun_2021, q, "2021")) %>%
...
grafico <- cmas_be(cmun_2012, q38, "2012") %>%
  ...
  #bind_rows(cmas_be(cmun_2014, q, "2014")) %>%
  #bind_rows(cmas_be(cmun_2015, q, "2015")) %>%
  #bind_rows(cmas_be(cmun_2016, q, "2016")) %>%
  bind_rows(cmas_be(cmun_2017, q44, "2017")) %>%
  #bind_rows(cmas_be(cmun_2018, q, "2018")) %>%
  #bind_rows(cmas_be(cmun_, q, "2019")) %>%
  #bind_rows(cmas_be(cmun_2020, q, "2020")) %>%
  #bind_rows(cmas_be(cmun_2021, q, "2021")) %>%
...
```{r Ceas_be, fig.cap = "Percentual de Conselhos Municipais que deliberam sobre critérios de Benefícios Eventuais - Brasil, 2012 - 2017 - 2022"}
