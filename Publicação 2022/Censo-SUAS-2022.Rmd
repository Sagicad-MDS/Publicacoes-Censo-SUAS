...
f_grafico_col_quantitativo = function(df, x, y, grupo){
  ...
  df %>%
    ...
    geom_text(aes(label = format(!! y, big.mark=".")),
              hjust = -0.01, 
...
f_grafico_col_quantitativo_flip = function(df, x, y, grupo){
  x <- enquo(x)
  y <- enquo(y)
  grupo <- enquo(grupo)
...
f_grafico_col_stack_percent = function(df, x, y, grupo, legend_nrow){
  ...
  df %>%
    ...
    geom_text(aes(label = label_percent(accuracy = 0.1, decimal.mark = ",")(!! y)), 
...
f_grafico_col_stack_percent_repel = function(df, x, y, grupo, legend_nrow){
  x <- enquo(x)
  y <- enquo(y)
  grupo <- enquo(grupo)
...
f_grafico_col_stack_percent_flip = function(df, x, y, grupo){
  x <- enquo(x)
  y <- enquo(y)
  grupo <- enquo(grupo)
  ...
  df %>%
    ggplot(aes(x = !! x, y = !! y)) +
    geom_col(aes(fill = !! grupo), position = position_stack(reverse = TRUE)) +
    geom_text(aes(label = label_percent(accuracy = 1, decimal.mark = ",")(!! y)), 
              position = position_stack(reverse = TRUE, vjust = .5)) +
    scale_y_continuous(expand = expand_scale(mult = c(0, .1))) +
    coord_flip() +
    theme(legend.position="bottom",
...
f_grafico_col_percent_flip = function(df, x, y, grupo, legend_nrow){
  ...
  df %>%
    ...
    theme(legend.position="bottom",
          ...
          axis.line.y = element_line(),
          panel.background = element_blank()) +
    guides(fill=guide_legend(nrow=legend_nrow, byrow=TRUE, reverse=TRUE))
}
...
# essa pergunta foi feita até 2019, após não se colocou mais
  ...
  f_grafico_col_stack_percent_repel(Categoria, Percentual, Nível, 4)
...
grafico<-tabela %>% 
  ...
  mutate(Categoria = factor(Categoria, levels = levels(ordenado))) %>% 
  mutate(Nível=str_wrap(Nível,width=22)) %>% 
  f_grafico_col_stack_percent_flip(Categoria, Percentual, Nível)
...
grafico<-tabela %>% 
  mutate(Categoria = factor(Categoria, levels = levels(ordenado))) %>% 
  f_grafico_col_quantitativo_flip(Categoria, n, Nível)
...
grafico<-tabela %>% 
  ungroup() %>% 
  f_grafico_col_percent_flip(Categoria, Percentual, Nível, 2)
...
grafico <- f_sim_nao(ge_2012,q8,"2012") %>%
  ...
  f_grafico_col_stack_percent_repel(Categoria, Percentual, Nível, 1)
...
grafico<-tabela %>% 
  mutate(Categoria = factor(Categoria, levels = levels(ordenado))) %>% 
  mutate(Ano = factor(Ano, levels = rev(levels(ordenado2)))) %>% 
  f_grafico_col_quantitativo_flip(Categoria, n, Ano)
...
grafico <- ord_despesa(ge_2012, q44, "2012") %>%
 ...
 f_grafico_col_stack_percent_repel(Categoria, Percentual, Nível, 2)
