...
f_grafico_col_percent = function(df, x, y, grupo, legend_nrow){
  x <- enquo(x)
  y <- enquo(y)
  grupo <- enquo(grupo)
...
grafico<-tabela %>% 
  ...
  f_grafico_col_percent(Situacao, Percentual, Categoria, 4)
...
grafico <- f_maior0_regiao_ano(cras_2007, `Acesso a Internet`, ibge, "2007") %>% 
  ...
  f_grafico_col_percent(Região, Percentual, Ano, 1)
...
grafico <- f_selecao_regiao_ano (creas_2022, q52_1, "Sim", Região, "Povos Indígenas") %>%
  ...
  f_grafico_col_percent(Região, Percentual, Ano, 2)
...
grafico<-tabela %>% 
  ...
  mutate(Categoria = factor(Categoria, levels = levels(ordenado))) %>% 
  mutate(Nível=str_wrap(Nível,width=22)) %>% 
  f_grafico_col_stack_percent_flip(Categoria, Percentual, Nível, 2)
...
```
  ...
  <!-- f_grafico_col_quantitativo(x, y, grupo) -->
  <!-- f_grafico_col_quantitativo_flip(x, y, grupo) -->
  <!-- f_grafico_col_percent(Região, Percentual, Ano, 1) -->
  <!-- f_grafico_col_percent_flip(x, y, grupo, legend_nrow) -->
  <!-- f_grafico_col_stack_percent(Categoria, Percentual, Nível, 1) -->
  <!-- f_grafico_col_stack_percent_flip(Categoria, Percentual, Nível, 1) -->
