...
Quanto a presença das unidades do Cadastro Único, os dados do Censo SUAS 2022 indicam a existência de 2.892 unidades exclusivas de Cadastro Único\footnote{O formulário do Cadastro Único teve sua primeira versão em 2020, assim todos os dados referentes a este questionário terão o marco deste ano de inicio}.
...
O PAIF compõe um conjunto de atenções conjugada a seguranças sociais refereciadas a undiade de CRAS. De acordo com Tipificação Nacional dos Serviços Socioassistenciais, o PAIF "consiste no trabalho social com família, de caráter contínuado, com a finalidade de fortalecer as função protetiva das famílias, provenir a ruptura dos seus vínculos, promover seu acesso e usufruto do direito".
...
A tônica do vínculo é um fator de Segurança Social, assim o SCFV no ambito do SUAS é um serviço tipificado e realizado através de grupos na qual objetiva garantir aquisições progressivas aos cidadãos usuários do SUAS. Trata-se de serviço complementar ao trabalho social com família e atua de forma preventiva a ocorrências de situações de risco social. De acordo com o \cref{fig:CRAS-SCFV}, 82% dos CRAS possuem a oferta deste serviço na sua unidade.

<!-- Há também esta oferta por meio de Unidades de Centro de Convivência conforme já mencionado (\Cref{fig:quantitativo-CentroConv}). -->
...
Em relação a atendimento Povos e comunidades Tradicionais Em 2018, 31% dos CRAS informaram que possue povos e comunidades tradicionais em seu território de abrangência. Esse dado avança em relação ao ano de 2012. \footnote{a partir de 2019 a pergunta é alterada}
...
grafico <- f_sim_nao(cras_2012, q40, "2012") %>%
  ...
  #bind_rows(f_sim_nao(cras_2019, q46, "2019")) %>%
  #bind_rows(f_sim_nao(cras_2020, q64, "2020")) %>%
    #bind_rows(f_sim_nao(creas_2021, q20, "2021")) %>%
  #bind_rows(f_sim_nao(cras_2022, q65_1_1, "2022")) %>%
...
Em relação a especifidade desse público atendidos pelo PAIF e PAEF, destaca-se que há registros de atendimento tanto nos CRAS como nos CREAS. Dados do Censo SUAS 2022 sinaliza que os povos mais atendiemtnos em nivel de Brasil são Indígenas e Quilombolas. O (\Cref{fig:CRAS-tradicionais}) traz estes dados por regiões.
...
# fazer grafico abaixo com questão 16 - censo suas 2022

```{r paef, fig.cap = "Percentual de CREAS que realizam Serviço de proteção social Especial para Pessoas com Deficiência, Idosas e suas Famílias, 2018 a 2022", fig.footer = "Fonte: MDS, Censo SUAS."}

creas_paefi = function(df, q, ano){
  q <- enquo(q)
  ano <- quo_name(ano)
...
tabela <- creas_paefi(creas_2018,q35,"2018") %>%
  bind_rows(creas_paefi(creas_2019, q33, "2019")) %>%
  bind_rows(creas_paefi(creas_2020, q38, "2020")) %>%
  bind_rows(creas_paefi(creas_2021, q38, "2021")) %>%
  bind_rows(creas_paefi(creas_2022, q38, "2022")) %>%
