...
inverso ao das gestões Estaduais. Observa-se através (\Cref{fig:sec-munic-exc}) um aumento das
secretarias exclusivas de Assistência Social ao longo dos anos até o período de 2019.
...
prioridades para o aperfeiçoamento institucional é possuir na
estrutura administrativa das seguintes areas:
...
Observa-se em 2022 todas as areas avançaram formalmente nos últimos 10 anos. 
As três areás formalmente mais instituídas são Proteção Social Básica,
Proteção Social Especial com 88,46% estados com as subdivisões administrativas e Gestão do SUAS com 84,62% constituídas formalmente \Cref{fig:uf_subd}.
...
```{r munic_sub, fig.cap = "Percentual segundo estrutura administrativa formal da Secretaria estadual de Assistência Social - Brasil, 2012 - 2017 - 2022", fig.footer = "Fonte: MDS, Censo SUAS."}
...
```{r municipais-constituicao-subdivisoes, fig.cap = "Distribuição dos órgãos gestores municipais segundo constituição/formalização de subdivisões administrativas - Brasil, 2022", fig.footer = "Fonte: MDS, Censo SUAS,eval=FALSE"}
...
Estadual de Assistência Social (CEAS).Os dados (\Cref{fig:PEAS}) sinalizam que a
partir de 2020 há um descrecimo desta atualização com devida aprovação do conselho.
...
grafico <- PEAS(ge_2012,q8,"2012") %>%
  bind_rows(PEAS(ge_2013,q9,"2013")) %>%
  bind_rows(PEAS(ge_2014,q9,"2014")) %>%
  bind_rows(PEAS(ge_2015,q9,"2015")) %>%
    
  bind_rows(PEAS(ge_2020,q5,"2020")) %>%
  bind_rows(PEAS(ge_2021,q5,"2021")) %>%
  bind_rows(PEAS(ge_2022,q5,"2022")) %>%
...
```{r plan_apoio_tec, fig.cap = "Percentual de Estados que possuiem Plano de Apoio Técnico pactuado na CIB - Brasil, 2013 a 2022", fig.footer = "Fonte: MDS, Censo SUAS"}
...
grafico <- plan_apoio(ge_2013, q85, "2013") %>%
   ...
   bind_rows(plan_apoio(ge_2019, q18, "2019")) %>%
...
```{r munic_vit_estadual, fig.cap = "Percentual de Municpioios que informam receber visitas de Apoio Técnico do Estado - Brasil, 2020 a 2022", fig.footer = "Fonte: MDS, Censo SUAS"}
...
grafico <- visit_estadual(gm_2020, q7_5, "2020") %>%
  bind_rows(visit_estadual(gm_2021, q7_5, "2021")) %>%
  bind_rows(visit_estadual(gm_2022, q7_5, "2022")) %>%
  
  #a partir de 2020 a pergunta foi alterada, impossibilizanto linha historica
...
grafico <- ge_cofinanciamento(ge_2012, q64_0, "2012") %>%
  ...
  bind_rows(ge_cofinanciamento(feas_2018, q_6, "2018")) %>% 
  bind_rows(ge_cofinanciamento(feas_2019, q9, "2019")) %>% 
  bind_rows(ge_cofinanciamento(feas_2020, q9, "2020")) %>%
  bind_rows(ge_cofinanciamento(feas_2021, q9, "2021")) %>%
...
grafico <- conc_munic(gm_2021, q72_1_1, "2021") %>%
  ...
  #2020 a base não foi possivel gerar
  #dados anteriores a 2019 sao perguntados de forma diferente
...
grafico <- conc_estado(ge_2013, q68a_1, "2013") %>%
    bind_rows(conc_estado(ge_2014, q22a_1, "2014")) %>%
    bind_rows(conc_estado(ge_2015, q15a_1, "2015")) %>%   
    bind_rows(conc_estado(ge_2016, q11_1_1, "2016")) %>%
    ...
    bind_rows(conc_estado(ge_2021, q36_1, "2021")) %>%
