...
<!--2012 não estar lendo a questão q12 que seria a correta, ler apenas q 13 -->.
...
Em relação a realização de reuniões pelos conselhos municipais, o (\Cref{fig:qtdcmas_reuniao})  sinaliza que 42% dos CMAS realizaram menos 8 reuniões no ano, sendo 14% deles inferior a 5 reuniões. A periodicidade mensal de reuniões devem estar prevista em regimento interno do respectivo conselho conforme é sinalizado na resolução CNAS Nº 100/2023.
...
### USO DO IGD PARA APOIO AO CONTROLE SOCIAL E INSTÂNCIA DE CONTROLE SOCIAL DO BOLSA FAMÍLIA
...
```{r ceas_plano, fig.cap = "Conselhos estaduais que deliberaram e/ou debatem sobre Plano de Assistência Social", eval=FALSE}
...
Em relação aos conselhos municipais, dados do censo suas de 2022 sinaliza que 82% dos CMAS informaram debater sobre plano municipal de assistência social. O (\Cref{fig:cmas_plano,})traz a referencia destas informações a partir do censo suas de 2012 conforme pode ser observado abaixo. \footnote{Até 2018 a pergunta é sobre a deliberação do Plano de Assistência Social pelo Conselho, a partir de 2018 a modalidade da pergunta é alterada e, passar a ser se o conselho debate sobre Plano de Assistencia Social. Apesar da diferença mantemos a linha historica por considerar a temática do Plano um item essencial para o controle social}.

```{r cmas_plano, fig.cap = "Conselhos municipais que deliberaram sobre a proposta anual de orçamento do executivo", eval=FALSE}
...
É papel do controle social o exercício democrático de acompanhar a gestão e avaliar a Política de Assistência Social. Assim, a aprovação do orçamento executivo ou Plano Plurianual de Assistência Social (PPA) deve ser exercida por esta instância. Os dados do (\Cref{fig:ceas_ppa}) evidencia que, no último censo suas 2012, 69% dos conselhos estaduais deliberaram sobre a proposta orçamentária, dado que corresponde ao moes percentual de 2012.  


```{r ceas_ppa, fig.cap = "Conselhos estaduais que deliberaram sobre a proposta anual do orçamento do executivo",eval=FALSE} 
...
```{r cmas_ppa, fig.cap = "Conselhos municipais que deliberaram sobre a proposta anual do orçamento do executivo",eval=FALSE}
...
```{r ceas_be, fig.cap = "Percentual de CEAS que aprovou em Resolução os critérios e prazos para acesso aos Benefícios Eventuais",eval=FALSE} 
...
```{r cmas_be, fig.cap = "Percentual de CMAS que aprovou em resolução os critérios e prazos para acesso aos Benefícios Eventuais",eval=FALSE} 

 grafico <- f_sim(cmun_2012, q38, "2012") %>%
#   bind_rows(f_sim_nao(cmun_2013, q39, "2013")) %>%
...
```{r usu_ceas, fig.cap = "Representantes de usuárias(os) nos Conselhos Estadual de Assistência Social", fig.footer = "Fonte: MDS, Censo SUAS.",eval=FALSE} 
...
```{r usu_cmun, fig.cap = "Representantes de usuárias(os) nos Conselhos Municipais de Assistência Social", fig.footer = "Fonte: MDS, Censo SUAS.",eval=FALSE} 
