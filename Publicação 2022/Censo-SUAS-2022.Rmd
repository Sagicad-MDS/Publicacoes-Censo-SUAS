...
No que se refere aos municípios, esse cenário nacional se configurou como contrário. Observa-se um aumento das secretarias exclusivas de Assistência Social ao longo dos anos, com exceção das regiões (complementar depois que rodar 2022, mas tem esse dado em 2022?) (\Cref{fig:secretarias-municipais-exclusivas}).
...
# Censo 2022:
  mutate(Nível = case_when(Nível <= 2012~"Até 2012",
                           Nível <= 2017~"Entre 2013 e 2017",
                           Nível <= 2021~"Entre 2018 e 2021",
                           TRUE ~ as.character(Nível))) %>% 
  mutate(Nível = factor(Nível, levels = rev(c("Até 2012","Entre 2013 e 2017", "Entre 2018 e 2021", "2022")))) %>% 
#-------------------
  group_by(Categoria, Nível) %>% 
  summarise(n=n()) %>% 
  group_by(Categoria) %>% 
...
<!-- para o ano de 2022 não teve a pergunta sobre PMAS, fazer apenas Estadual - questão q5 _2022 - grafico pizza  --> 
...
<!-- Quando se observa a atualização do PMAS por região, verifica-se que 40,3% dos municípios da região Sudeste atualizaram seus Planos em 2016. Nas demais regiões, a maior parte dos municípios atualizaram seus Planos de 2 a 5 anos atrás: 45,6% dos municípios da região Norte; 40,6% dos municípios da região Nordeste; 65,5% dos municípios da região Sul e 50,5% dos municípios da região Centro-Oeste (\Cref{fig:municipios-atualizacao-PMAS}). -->
