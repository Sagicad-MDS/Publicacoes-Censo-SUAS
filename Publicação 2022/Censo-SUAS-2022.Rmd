...
O número absoluto e o percentual de CRAS com acesso à internet aumentam desde 2007 chegando no último ano desta sériie com mais de 99% das unidades com computadores com acesso a internet conforme (\Cref{fig:CRAS-internet-percentual}).
...
```{r centro-conv-natureza, fig.cap = "Quantitativo de Centros de Convivência por Natureza da Unidade - BRASIL; 2016 a 2022", fig.footer = "Fonte: MDS, Censo SUAS.", fig.footer = "Fonte: MDS, Censo SUAS."}
...
```
Em relação aos Conselhos Estaduais de Assistência Social o quadro abaixo (\Cref{fig:qtd-ceas}) sinalzia que 100% dos conselhos responderam ao formulário do Censo SUAS. 
...
if(gera.graficos.office) grafico %>% graph2office(file="graficos", append = TRUE)
```
Em relação aos Conselhos Estaduais, atualmente 100% informam possuir sede específica para funcionamento do controle social no SUAS conforme pdoe ser observado abaixo (\Cref{fig:ceas_sede}) 
...
```{r ceas_se, fig.cap = "Percentual de Conselhos Estaduais que possuem secretários (as) executivos exclusivamente no Conselho"}
...
```{r qtdcmas_reuniao, fig.cap = "Proporção conselhos municipais quando a realização de reuniões plenárias do Conselho (ordinárias e extraordinárias), eval=FALSE"}
...
# Censo 2022:
  mutate(Nível = case_when(Nível <= 5~"Até 5",
                           ...
                           Nível <= 12~"Acima 12",
                           TRUE ~ as.character(Nível))) %>% 
  mutate(Nível = factor(Nível, levels = rev(c("Até 5","Entre 5 e 8", "Entre 8 e 12", "Acima 12")))) %>% 
#-------------------
  group_by(Categoria, Nível) %>% 
  summarise(n=n()) %>% 
  group_by(Categoria) %>% 
...
```{r qtdceas_reuniao, fig.cap = "Proporção conselhos estaduais quanto a realização de reuniões plenárias no ano anterior, eval=FALSE"}
...
Temas debatidos nos conselhos municipais ...


```{r temas_cmas, fig.cap = "Percentual de temas discutidos pelos Conselhos Municipais de Asssistência Social", fig.footer = "Fonte: MDS, Censo SUAS."}

temas_cmas= function(df, q, ano, cod_categoria){
  q <- enquo(q)
  ano <- quo_name(ano)
  cod_categoria <- quo_name(cod_categoria)

#2022
  categoria <- case_when(!! cod_categoria == "1"~"Plano de Ação",
                         !! cod_categoria == "2"~"Demonstrativo de Execução Físico-Financeira",
                         !! cod_categoria == "3"~"Plano de Assistência Social",
                         !! cod_categoria == "4"~"Plano Plurianual (PPA), Lei de Diretrizes Orçamentárias (LDO) e/ou Lei Orçamentária Anual (LOA)",
                         !! cod_categoria == "5"~"Deliberações da(s) Conferência(s)",
                         !! cod_categoria == "6"~"Planejamento das atividades do conselho",
                         !! cod_categoria == "7"~"Acompanhamento do Programa Bolsa Família",
                         !! cod_categoria == "8"~"Acompanhamento do Benefício de Prestação Continuada",
                         !! cod_categoria == "9"~"Benefícios eventuais")
 ...
 tabela <- temas_cmas (cmun_2014,q29_1,"2014","1") %>% 
  bind_rows(temas_cmas(cmun_2014,q29_2,"2014","2")) %>%
  bind_rows(temas_cmas(cmun_2014,q29_3,"2014","3")) %>%
  bind_rows(temas_cmas(cmun_2014,q29_4,"2014","4")) %>%
  bind_rows(temas_cmas(cmun_2014,q29_6,"2014","5")) %>%
  bind_rows(temas_cmas(cmun_2014,q29_7,"2014","6")) %>%
  bind_rows(temas_cmas(cmun_2014,q29_8,"2014","7")) %>%
  bind_rows(temas_cmas(cmun_2014,q29_9,"2014","8")) %>%
  bind_rows(temas_cmas(cmun_2014,q29_11,"2014","9")) %>%
  
   
  bind_rows(temas_cmas(cmun_2018,q_34_2,"2018","1")) %>%
  bind_rows(temas_cmas(cmun_2018,q_34_3,"2018","2")) %>%
  bind_rows(temas_cmas(cmun_2018,q_34_4,"2018","3")) %>%
  bind_rows(temas_cmas(cmun_2018,q_34_5,"2018","4")) %>%
  bind_rows(temas_cmas(cmun_2018,q_34_6,"2018","5")) %>%
  bind_rows(temas_cmas(cmun_2018,q_34_7,"2018","6")) %>%
  bind_rows(temas_cmas(cmun_2018,q_34_8,"2018","7")) %>%
  bind_rows(temas_cmas(cmun_2018,q_34_9,"2018","8")) %>%
  bind_rows(temas_cmas(cmun_2018,q_34_10,"2018","9")) %>%

   
  bind_rows(temas_cmas(cmun_2022, q33_2,"2022","1")) %>%
  bind_rows(temas_cmas(cmun_2022, q33_3,"2022","2")) %>%
  bind_rows(temas_cmas(cmun_2022, q33_4,"2022","3")) %>%
  bind_rows(temas_cmas(cmun_2022, q33_5,"2022","4")) %>%
  bind_rows(temas_cmas(cmun_2022, q33_6,"2022","5")) %>%
  bind_rows(temas_cmas(cmun_2022, q33_7,"2022","6")) %>%
  bind_rows(temas_cmas(cmun_2022, q33_8,"2022","7")) %>%
  bind_rows(temas_cmas(cmun_2022, q33_9,"2022","8")) %>%
  bind_rows(temas_cmas(cmun_2022, q33_10,"2022","9")) %>%
...
Sobre os temas debatidos pelos conselhos estaduais de Assistência Social, destaca-se listagem de 9 itens na qual em comparação com 2014, \footnote{esse dado foi contabilizado a partir do ano de 2014})} todos os temas tiveram uma redução nas pautas dos conselhos. O grafico ainda mostra que o tema mais discutido é refente ao Plano de ação e o menos debatido é o acompanhamento do Benefício de Prestação continuada conforme (\Cref{fig:temas_ceas}).


```{r temas_ceas, fig.cap = "Percentual de temas discutidos pelos Conselhos Estaduais de Asssistência Social", fig.footer = "Fonte: MDS, Censo SUAS."}

temas_ceas= function(df, q, ano, cod_categoria){
  q <- enquo(q)
  ano <- quo_name(ano)
  cod_categoria <- quo_name(cod_categoria)
...
#2022
 ...
 tabela <- temas_ceas (cest_2014,q29_1,"2014","1") %>% 
  bind_rows(temas_ceas(cest_2014,q29_2,"2014","2")) %>%
  bind_rows(temas_ceas(cest_2014,q29_3,"2014","3")) %>%
  bind_rows(temas_ceas(cest_2014,q29_4,"2014","4")) %>%
  bind_rows(temas_ceas(cest_2014,q29_6,"2014","5")) %>%
  bind_rows(temas_ceas(cest_2014,q29_7,"2014","6")) %>%
  bind_rows(temas_ceas(cest_2014,q29_8,"2014","7")) %>%
  bind_rows(temas_ceas(cest_2014,q29_9,"2014","8")) %>%
  bind_rows(temas_ceas(cest_2014,q29_11,"2014","9")) %>%
  
   
  bind_rows(temas_ceas(cest_2018,q_34_2,"2018","1")) %>%
  bind_rows(temas_ceas(cest_2018,q_34_3,"2018","2")) %>%
  bind_rows(temas_ceas(cest_2018,q_34_4,"2018","3")) %>%
  bind_rows(temas_ceas(cest_2018,q_34_5,"2018","4")) %>%
  bind_rows(temas_ceas(cest_2018,q_34_6,"2018","5")) %>%
  bind_rows(temas_ceas(cest_2018,q_34_7,"2018","6")) %>%
  bind_rows(temas_ceas(cest_2018,q_34_8,"2018","7")) %>%
  bind_rows(temas_ceas(cest_2018,q_34_9,"2018","8")) %>%
  bind_rows(temas_ceas(cest_2018,q_34_10,"2018","9")) %>%

 
  bind_rows(temas_ceas(cest_2022, q33_2,"2022","1")) %>%
  bind_rows(temas_ceas(cest_2022, q33_3,"2022","2")) %>%
  bind_rows(temas_ceas(cest_2022, q33_4,"2022","3")) %>%
  bind_rows(temas_ceas(cest_2022, q33_5,"2022","4")) %>%
  bind_rows(temas_ceas(cest_2022, q33_6,"2022","5")) %>%
  bind_rows(temas_ceas(cest_2022, q33_7,"2022","6")) %>%
  bind_rows(temas_ceas(cest_2022, q33_8,"2022","7")) %>%
  bind_rows(temas_ceas(cest_2022, q33_9,"2022","8")) %>%
  bind_rows(temas_ceas(cest_2022, q33_10,"2022","9")) %>%
...
```{r ceas_icspbf, fig.cap = "Percentual de Conselhos Estaduais de Assistência Social que são instancia de controle Social do PBF", eval=FALSE}
...
```{r usu_cmun, fig.cap = "Representantes de usuários (as) nos Conselhos Municipais de Assistência Social", fig.footer = "Fonte: MDS, Censo SUAS."}
...
```{r usu_ceas, fig.cap = "Representantes de usuários (as) nos Conselhos Estadual de Assistência Social", fig.footer = "Fonte: MDS, Censo SUAS."}
...
```{r trab_cmun, fig.cap = "Representantes de trabalhadores (as) nos Conselhos Municipais de Assistência Social", fig.footer = "Fonte: MDS, Censo SUAS."}
...
```{r trab_rep, fig.cap = "Representantes de trabalhadores (as) nos Conselhos Estadual de Assistência Social", fig.footer = "Fonte: MDS, Censo SUAS."}
...
```{r pub_ceas, fig.cap = "Percentual de conselhos municipais que publicam as resoluções em diário oficial", fig.footer = "Fonte: MDS, Censo SUAS", eval=FALSE}

pub_ceas  = function(df, q, ano){
  q <- enquo(q)
  ano <- quo_name(ano)
...
grafico <- pub_ceas(cest_2017, q36, "2017") %>%  
# bind_rows(estados_sec_exc(ge_2013, q1, "2013")) %>%
# bind_rows(estados_sec_exc(ge_2014, q1, "2014")) %>%
# bind_rows(estados_sec_exc(ge_2015, q1, "2015")) %>%
# bind_rows(estados_sec_exc(ge_2016, q1, "2016")) %>%  
# bind_rows(estados_sec_exc(ge_2017, q36, "2017")) %>%  
bind_rows(pub_ceas(cest_2018, q_29,"2018")) %>%  
bind_rows(pub_ceas(cest_2019, q29, "2019")) %>%  
bind_rows(pub_ceas(cest_2020, q27, "2020")) %>%  
bind_rows(pub_ceas(cest_2021, q27, "2021")) %>%  
bind_rows(pub_ceas(cest_2022, q27, "2022")) %>%    
