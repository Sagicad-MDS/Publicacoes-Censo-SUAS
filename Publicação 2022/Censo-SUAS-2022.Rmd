...
inverso ao das gestões Estaduais. Observa-se através (\Cref{fig:sec-munic-exc}) um aumento de 217 municípios (1,5 pontos percentuais) com secretarias exclusivas de Assistência Social do período de 2012 a 2019.
...
Observa-se avanço em todas as areas formalmente nos últimos 10 anos. 
As três áreas mais instituídas são: Proteção Social Básica,
Proteção Social Especial com 88,46% e Gestão do SUAS com 84,62% \Cref{fig:uf_subd}.
...
Quando se observa também as areas instituídas informalmente conforme \Cref{fig:estados-constituicao-subdivisoes}, apenas os setores da Gestão do SUAS,Gestão do trabalho e regulação do SUAS não estão contituídas em 100% dos estados.  
...
A \Cref{fig:munic_sub} mostra crescimento formal de todas as areas administrativas. As areas
do SUAS com maior percentual de constituição formal \footnote{A
...
Social (LOAS) - nº 8.742, de 07/.12.1993} que dispõe sobre esta organização são respectivamente:
...
Ao considerar a instituição informal, o \Cref{fig:municipais-constituicao-subdivisoes} 
sinaliza que a Gestão do trabalho e Vigilância Socioassistencial
são os mais altos percentuais de informalidade e não constituição. 
...
nesta instância de comissão intergestores bipartite.
...
Abaixo (\Cref{fig:munic_vit_estadual}) os dados percentuais mostram um aumento gradual dos municípios que informam receber
visitas da gestão estadual para apoio técnico.Entretanto 62,5% informam não ter recebido visita de apoio técnico no último ano.
...
if(gera.graficos.office) grafico %>% graph2office(file="graficos", append = TRUE)
```
<!-- Valdson para este grafico stados-cofinanciamento-municipios abaixo seria possível a gente não contar o DF? pois eles nesta configuração não se aplica}). -->

O modelo de gestão do SUAS preve cofinanciamento compartilhado entre os
três entes federados.\footnote{Art. 30-A.  O cofinanciamento dos serviços, programas, projetos e benefícios eventuais, no que couber, e o aprimoramento da gestão da política de assistência social no Suas se efetuam por meio de transferências automáticas entre os fundos de assistência social e mediante alocação de recursos próprios nesses fundos nas 3 (três) esferas de governo.}.
...
(Fundo a Fundo). Sobre esse item, observa-se que em 2012, 26,63% (8) dos
estados realizavam cofinanciamento na modalidade de apenas Fundo a Fundo
e 25,9% (7) apenas por convenio. Dado que avançou ao longo dos anos,
...
recomenda-se que este tenha amplo conhecimento sobre a política. O gráfico
abaixo (\Cref{fig:estado_ord_despesa}) sinaliza que
73% dos gestores estaduais são ordenadores de despesa do fundo
...
O (\Cref{fig:munic_ord_despesa}) também mostra que os prefeitos (as)
...
No que se refere a Educação Permanente, nota-se um aumento dos estados que 
possuem Núcleo de Educação Permante, a (\Cref{fig:NUEP}). 

O censo de 2022 sinaliza 88,5% (23) dos Estados possuem esta instancia colegiada 
de maneira formal. Trata-se de um foro contituída pela participação e cooperação 
institucionalizada de gestores, trabalhadores, usuários, conselheiros de 
assistência social, e instituições de ensino, pesquisa e extensão. 
O objetivo é deliberar sobre a implementação continuada da Política de educação 
permanente do SUAS.

```{r NUEP, fig.cap = "Percentual de Estados que possuem Núcleo de Educação Permanete, 2020 a 2022", fig.footer = "Fonte: MDS, Censo SUAS"}
...
Em relação ao Plano de Educação Permanente, de acordo com censo suas de 2021, 
13,9% (xx) dos municípios possuem este plano. Esse percentual, como pode ser 
observado (\Cref{fig:PNEP_Munic}) não avançaou muito nos últimos anos. 
\footnote{Para o ano de 2022 essa pergunta foi extinta}.


```{r PNEP_Munic, fig.cap = "Percentual de municípios que possuem Plano de Capacitação e Educação Permanente, segundo grandes regiões - Brasil; 2015, 2017 e 2022", fig.footer = "Fonte: MDS, Censo SUAS"}

PNEP_Munic  = function(df, q, ano){
  q <- enquo(q)
  ano <- quo_name(ano)
...
grafico <- PNEP_Munic(gm_2015, q40, "2015") %>%
    bind_rows(PNEP_Munic(gm_2016, q20, "2016")) %>%  
    bind_rows(PNEP_Munic(gm_2017, q31, "2017")) %>%
    bind_rows(PNEP_Munic(gm_2018, q_51, "2018")) %>%  
    bind_rows(PNEP_Munic(gm_2019, q52, "2019")) %>%
    #bind_rows(PNEP_Munic(gm_2020, q73, "2020")) %>%
    bind_rows(PNEP_Munic(gm_2021, q73, "2021")) %>%
...
Em relação a realização de concurso público para trabalahdores (as) de nível 
superior, percebe-se que os anos de 2013 e 2015 foram os anos com maior número 
de municípios realizaram, 26,7% e 17,2% respectivamente. 
...
grafico <- conc_munic(gm_2012, q43a_1, "2012") %>%
  bind_rows(conc_munic(gm_2013, q36_8, "2013")) %>%
  bind_rows(conc_munic(gm_2014, q51_1_1, "2014")) %>%
  bind_rows(conc_munic(gm_2015, q38_1a, "2015")) %>%
  bind_rows(conc_munic(gm_2016, q19_1, "2016")) %>%
  bind_rows(conc_munic(gm_2017, q30.1.1, "2017")) %>%
  bind_rows(conc_munic(gm_2018, q_50_1, "2018")) %>%
  bind_rows(conc_munic(gm_2019, q51_1_1, "2019")) %>%
...
grafico <- conc_estado(ge_2013, q68a_1, "2013") %>%
    ...
    bind_rows(conc_estado(ge_2021, q46_1_1, "2021")) %>%
...
O gráfico abaixo sinaliza que no âmbito da gestão municipal, as ações de 
levantamento de famílias para atualização e inclusão cadastral é a mais 
realizada. A menor proporção se refere a ações de elaboração de análises 
utilizando dados do Cadastro Único (\Cref{fig:gestao_cad}).
...
```{r gestao_cad, fig.cap = "Ações desenvolvidas no ambito pelos postos do Cadastro Único - Brasil, 2021 e 2022", fig.footer = "Fonte: MDS, Censo SUAS."}
...
O gráfico (\Cref{fig:acoes_cad}) sinaliza que as ações de esclarecimento de 
dúvidas sobre serviços, programas e outros é realizada pela grande maioria das
unidades do cadastro único (96,82%). Já as ações de agendamento para
atendimento é realizada por 64,35% dos unidades, com redução ao longo
dos anos.
...
A visita domiciliar permite uma aproximação com aspectos cotidiano das famílias,
trata-se de uma forma de coleta das informações por meio da ida até as famílias, 
essa ação objetiva assegurar o acesso a inclusão ou atualização na residencia da
...
```{r atend-cadunico, fig.cap = " Percentual de postos do Cadastro Único - atendimento -2020 - 2021 - 2022", fig.footer = "Fonte: MDS, Censo SUAS",eval=FALSE}
