...
creas_2018 <- read_excel("../Censo SUAS 2018/2 - CREAS/Dicionario_Censo_SUAS_2018_CREAS_dados_gerais_divulgacao.xlsx", sheet = 1)
...
grafico <- f_selecao_regiao_ano (gm_2012, q1, "Secretaria municipal exclusiva da área de Assistência Social", Região, "2012") %>%
  ...
  #bind_rows(f_selecao_regiao_ano(gm_2014, q1, "Secretaria municipal exclusiva da área de Assistência Social", Região, "2014")) %>% 
   ...
   #bind_rows(f_selecao_regiao_ano(gm_2022, q1, "Secretaria municipal exclusiva da área de Assistência Social", Região, "2022")) %>% 
...
Observa-se que meta chega em 2022 com 22 (85%) estados com as subdivisões administrativas das áreas de Proteção Social Básica, Proteção Social Especial e Gestão do SUAS constituídas formalmente. São 4 areas que não possuem 100% contituídas (formal e informal) sçao elas: Gestão do SUAS, Gestão do trabalho, Serviços e Beneficios e REgulação do SUAS. Esta última apesenta menor percentual de formalização, sendo 38,5% subdivisões constituídas formalmente conforme grafico abaixo (\Cref{fig:estados-constituicao-subdivisoes}).
...
O (\Cref{fig:municipais-constituicao-subdivisoes}) sinaliza que as areas do SUAS com maior percentual de constituição formal são respectivamente: Gestão do Cadastro Único e Bolsa Família, Gestão da Proteção Social Básica e Gestçao do SUAS.    
...
Em 2022, todos os estados informaram realizar alguma modalidade de apoio técnico aos municípios. Os maiores percentuais observados referiam-se ao Apoio técnico individualizado a municípios específicos, ofertado por 96,2% dos estados (25). Os menores percentuais eram referentes a Seminários 46,2%, (12). Outras formas, além das discriminadas eram ofertadas por 15,4% dos estados.

Entre 2013 e 2022 o percentual de estados que prestavam assessoramento técnico a distância aumentou, por sua vez, aumentou significativamente, passando de 42,3% (11) em 2013 para 92,3% (24) em 2022 (\Cref{fig:estados-formas-apoio}).
...
<!-- colocar região no gm2022 para poder rodar o grafico  --> 
...
Apesar destes avanços, há registros de não cofinanciamento estadual, na qual saiu de 25,93% () no ano de 2012 para 3,85% em 2022 (1) \footnote{o Estado do ACRE não focinancia os municípios)}.
...
Nota-se tambémq ue não há nenhum bloco quem que 100% dos estados cofinanciam 
...
```{r municipios-participacoes-CIB, fig.cap = "Percentual de municípios por número de participações em reuniões da CIB (Comissão Intergestores Bipartite) no ano de 2016, segundo grandes regiões - Brasil, 2017", fig.footer = "Fonte: MDS, Censo SUAS."}
...
```{r municipios-possuem-plano, fig.cap = "Percentual de municípios que possuem Plano de Capacitação e Educação Permanente, segundo grandes regiões - Brasil, 2015 a 2017", fig.footer = "Fonte: MDS, Censo SUAS."}
...
grafico <- f_cras_internet(cras_2007, v17, "2007") %>%
#  bind_rows(f_equip_imovel(cras_2008, situacao_imovel, "2008")) %>% 
#  bind_rows(f_equip_imovel(cras_2009, q5_situac_imovel, "2009")) %>% 
#  bind_rows(f_equip_imovel(cras_2010, q5, "2010")) %>% 
#  bind_rows(f_equip_imovel(cras_2011, q5, "2011")) %>%
...
```{r centro-conv, fig.cap = "Evolução de Centro de Convivência - Brasil, 2011 a 2022", fig.footer = "Fonte: MDS, Censo SUAS."}
...
```{r centro-conv-natureza, fig.cap = "Quantitativo de Centros de Convivência por Natureza da Unidade - BRASIL; 2014, 2018 e 2022", fig.footer = "Fonte: MDS, Censo SUAS."}
...
```{r CRAS-PSB, fig.cap = "Percentual de CRAS que realizam Serviço de Proteção Social Básica no Domicílio para Pessoas com deficiência e idosas", fig.footer = "Fonte: MDS, Censo SUAS."}
...
```{r CREAS-quantitativo, fig.cap = "Evolução do quantitativo de CREAS, 2012 - 2017 - 2022", fig.footer = "Fonte: MDS, Censo SUAS."}
...
```{r CREAS-abordagem-social, fig.cap = "Quantidade de CREAS que realizam o Serviço Especializado em Abordagem Social, 2015 - 2023", fig.footer = "Fonte: MDS, Censo SUAS."}
...
```{r centro-pop, fig.cap = "Evolução de Centro pop - 2011 a 2022."}

```

```{r cpop_acessibilidade, fig.cap = "Evolução percentual de Centros POP segundo condições de acessibilidade de acordo com a Norma da ABNT - Brasil; 2012, 2017 e 2022", fig.footer = "Fonte: MDS, Censo SUAS."}

#tabela <- equip_acessibilidade(cpop_2011, q8a, "2011", "1") %>%
#  bind_rows(equip_acessibilidade(cpop_2011, q8b, "2011", "2")) %>% 
#  bind_rows(equip_acessibilidade(cpop_2011, q8c, "2011", "3")) %>% 
#  bind_rows(equip_acessibilidade(cpop_2011, q8d, "2011", "4")) %>% 
tabela <- equip_acessibilidade(cpop_2012, q8_1, "2012", "1") %>%
  bind_rows(equip_acessibilidade(cpop_2012, q8_2, "2012", "2")) %>% 
  bind_rows(equip_acessibilidade(cpop_2012, q8_3, "2012", "3")) %>% 
  bind_rows(equip_acessibilidade(cpop_2012, q8_4, "2012", "4")) %>% 
  bind_rows(equip_acessibilidade(cpop_2017, q7.1, "2017", "1")) %>%
  bind_rows(equip_acessibilidade(cpop_2017, q7.2, "2017", "2")) %>% 
  bind_rows(equip_acessibilidade(cpop_2017, q7.3, "2017", "3")) %>% 
  bind_rows(equip_acessibilidade(cpop_2017, q7.4, "2017", "4")) %>% 
  bind_rows(equip_acessibilidade(cpop_2022, q7_1, "2022", "1")) %>%
  bind_rows(equip_acessibilidade(cpop_2022, q7_2, "2022", "2")) %>% 
  bind_rows(equip_acessibilidade(cpop_2022, q7_3, "2022", "3")) %>% 
  bind_rows(equip_acessibilidade(cpop_2022, q7_4, "2022", "4")) %>% 
  ungroup() %>% 
  mutate(Categoria=str_wrap(Categoria,width=34))
...
```{r cdia-evolucao, fig.cap = "Evolução de CENTRO DIA - 2015 a 2022", fig.footer = "Fonte: MDS, Censo SUAS."}
...
```{r unac-natureza, fig.cap = "Percentual de Unidades de Acolhimento segundo natureza da unidade - Brasil, 2014 a 2022", fig.footer = "Fonte: MDS, Censo SUAS."}
...
```{r equip-cadunico, fig.cap = "Percentual de CRAS, Centro de Convivência, CREAS e Centro Pop com locais para CadÚnico - 2018 - 2022",fig.footer = "Fonte: MDS, Censo SUAS."}

```

```{r postos-cadunico, fig.cap = "Evolução de Postos do Cadastro Único",fig.footer = "Fonte: MDS, Censo SUAS."}

```

## POVOS TRADICIONAIS

```{r povos-tradicionais, fig.cap = "Número de CRAS, CREAS que atendem povos e comunidades tradicionais - Brasil, 2014 - 2022",fig.footer = "Fonte: MDS, Censo SUAS."}
...
```{r be-local, fig.cap = "Local de concessão dos Benefícios Eventuais - 2014-2022", fig.footer = "Fonte: MDS, Censo SUAS."}
