...
f_grafico_col_quantitativo = function(df, x, y, grupo){
  x <- enquo(x)
  y <- enquo(y)
  grupo <- enquo(grupo)
  ...
  df %>%
    ...
    geom_text(aes(label = format(!! y, big.mark=".")), hjust = -0.01, position = position_dodge(width = 0.9), angle = 90) +
...
f_nomex_somay_nomegrupo_semDF = function(df, nomex, somay, nomegrupo, uf){
...
f_stack_col_percentual  = function(df, eixo_x, n, grupo){
  eixo_x <- enquo(eixo_x)
  n <- enquo(n)
  grupo <- enquo(grupo)

  df %>%
    mutate(!! grupo := factor(!! grupo, levels = unique(!! grupo))) %>% 
    arrange(!! grupo) %>% 
    group_by(!! eixo_x) %>% 
    mutate(Percentual = !! n/sum(!! n), Posição = (cumsum(!! n/sum(!! n)) - 0.5 * !! n/sum(!! n)))
}
...
Em relação as informações de unidades de Postos do Cadasto Único e o atendimento de Grupos Tradicionais e Especificos - GPTEs, destaca-se através (\Cref{fig:gptes-cadunico}) o percertual de unidades de postos do cadastro único que informam atender no período de 2020 a 2022. 
...
grafico <- f_sim_nao(posto_cadunico_2020, q32_1, "2020") %>%
        bind_rows(f_sim_nao(posto_cadunico_2021, q32_1, "2021"))%>%
        bind_rows(f_sim_nao(posto_cadunico_2022, q32_1, "2022"))%>%
...
#  bind_rows(f_quantitativo_regiao_ano(cras_2016, Região, "2016")) %>% 
  ...
  f_grafico_col_quantitativo(Região, n, Ano)
...
grafico <- f_nomex_somay_nomegrupo_semDF(ge_2010, "2010", q34_1_4, "Estatutários", UF) %>%  
  bind_rows(f_nomex_somay_nomegrupo_semDF(ge_2010, "2010", q34_2_4, "Celetistas", UF)) %>%  
  bind_rows(f_nomex_somay_nomegrupo_semDF(ge_2010, "2010", q34_3_4, "Somente Comissionados", UF)) %>%  
  bind_rows(f_nomex_somay_nomegrupo_semDF(ge_2010, "2010", q34_4_4, "Outros Vínculos", UF)) %>%

  bind_rows(f_nomex_somay_nomegrupo_semDF(ge_2011, "2011", q55a4, "Estatutários", UF)) %>%  
  bind_rows(f_nomex_somay_nomegrupo_semDF(ge_2011, "2011", q55b4, "Celetistas", UF)) %>%  
  bind_rows(f_nomex_somay_nomegrupo_semDF(ge_2011, "2011", q55c4, "Somente Comissionados", UF)) %>%  
  bind_rows(f_nomex_somay_nomegrupo_semDF(ge_2011, "2011", q55d4, "Outros Vínculos", UF)) %>%
 
  f_stack_col_percentual(Eixo_x, Eixo_y, Grupo) %>%

  ggplot(aes(x = Eixo_x, y = Percentual, group = Grupo)) +
  geom_col(aes(fill = Grupo), position = position_stack(reverse = TRUE)) +
  geom_text(aes(y=Posição, label = percent(Percentual))) +
  scale_y_continuous(expand = expand_scale(mult = c(0, .1))) +
  theme(legend.position="bottom",
...
# =======
# grafico <- f_sim_nao(cmun_2012, q13, "2012") %>%
...
grafico <- f_sim_nao(cest_2012, q35_1, "2012") %>%
  #bind_rows(f_sim_nao(cest_2013, q8, "2013")) %>%
  #bind_rows(f_sim_nao(cest_2014, q8, "2014")) %>%
  #bind_rows(f_sim_nao(cest_2015, q8, "2015")) %>%
  #bind_rows(f_sim_nao(cest_2016, q38, "2016")) %>%
   bind_rows(f_sim_nao(cest_2017, q38, "2017")) %>%
  #bind_rows(f_sim_nao(cest_2018, q8, "2018")) %>%
  #bind_rows(f_sim_nao(cest_2019, "2019")) %>%
  #bind_rows(f_sim_nao(cest_2020, q8, "2018")) %>%
  #bind_rows(f_sim_nao(cest_2021, q8, "2018")) %>%
   bind_rows(f_sim_nao(cest_2022, q33_4, "2022")) %>%
...
grafico <- f_quantitativo_niveis(idcras_2022, `Estrutura Física`, "Estrutura Física", niveis_ID) %>% 
  ...
  mutate(Eixo_X = factor(Eixo_X, levels = c("Estrutura Física",
                                            "Recursos Humanos",
                                            "Serviços & Benefícios"))) %>% 
  f_grafico_col_quantitativo(Eixo_X, n, Grupo)
