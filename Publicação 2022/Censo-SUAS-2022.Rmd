...
unac_2020 <- read_excel("../Censo SUAS 2020/Censo_SUAS_2020_DG_Acolhimento.xlsx", sheet = 1)

unacrh_2020 <- read_excel("../Censo SUAS 2020/Censo_SUAS_2020_RH_Acolhimento.xlsx", sheet = 1)
...
Em relação aos programas próprios de transferência de renda executados pelos estados, percebe-se um aumento ao longo dos anos na qual em 2022, 76,9% dos estados informaram que possuem conforme o \cref{fig:be_uf_renda}.
...
grafico <- f_sim_nao(cras_2012, q19_1a, "2012") %>%
  #bind_rows(f_sim_nao(cras_201, q19, "2013")) %>%
  bind_rows(f_sim_nao(cras_2014, q19, "2014")) %>%
...
```{r povos-tradicionais_cras, fig.cap = "Percentual de CRAS que possuem povos e comunidades tradicionais no território de abrangencia, 2012 - 2022", fig.footer = "Fonte: MDS, Censo SUAS."}

grafico <- f_sim_nao(cras_2012, q40, "2012") %>%
  bind_rows(f_sim_nao(cras_2013, q51, "2013")) %>%
  bind_rows(f_sim_nao(cras_2014, q52, "2014")) %>%
  bind_rows(f_sim_nao(cras_2015, q44, "2015")) %>%
  bind_rows(f_sim_nao(cras_2016, q37, "2016")) %>%
  bind_rows(f_sim_nao(cras_2017, q42, "2017")) %>%
  bind_rows(f_sim_nao(cras_2018, q_47, "2018")) %>%
...
Em relação as especifidades desse público atendidos pelo PAIF e PAEFI, destaca-se que há registros de atendimento tanto nos CRAS como nos CREAS.  Dados do Censo SUAS 2022 sinaliza que os povos mais atendidos em nível de Brasil são Indígenas e Quilombolas. O \Cref{fig:CRAS-tradicionais} traz estes dados por regiões.
...
O (\Cref{fig:paefi_creas}) mostra que cresceu, entre o período de 2019 a 2022, as os atendimentos de abuso e violência sexual, violência contra mulher e crianças e adolescentes bem como situações de negligência e abandono. O gráfico  também destaca que as situações de tráfico de pessoas tiveram aumento significativo neste período. 
...
Em relação a violação em decorrência de trabalho análogo a escravidão é definida como toda atividade forçada desenvolvida sob condições degradantes ou em jornadas exaustivas e quando a pessoa é impedida de deixar o seu local de trabalho \footnote{Artigo 149 do Código Penal}. O (\Cref{fig:paefi2_creas}) também trás as situações de trabalho infantil. Observa-se que a maior demanda atendida pelos CREAS, entre o período de 2020 e 2022, através do PAEFI é trabalho infantil. Entretanto as situações de situações de trabalho análogo a escravidão é presente em média 13% dos CREAS. 
...
#2022
 ...
 tabela <- paefi2_creas(creas_2022,q16_8_1,"2022","1") %>%
  ...
  # bind_rows(paefi2_creas(creas_2019,q16_8_1,"2019","1")) %>%
  # bind_rows(paefi2_creas(creas_2019,q16_8_2,"2019","2")) %>%
  # bind_rows(paefi2_creas(creas_2019,q16_8_3,"2019","3")) %>%
  # bind_rows(paefi2_creas(creas_2019,q16_8_4,"2019","4")) %>%
  # bind_rows(paefi2_creas(creas_2019,q12_8_1,"2019","6")) %>%
...
if(gera.graficos.office) grafico %>% graph2office(file="graficos", append = TRUE)
```
As situações de rua atendidas pelo PAEFI tiveram uma redução de atendimentos em todos os públicos sinalizados através (\Cref{fig:paefi3_creas}), durante período de 2019 a 2022. Tal fato pode ser atribuído ao período da pandemia \footnote{O Censo SUAS 2022 é referente aos dados do ano de 2021}. Destaca-se também que nos municípios maiores a presença de Centro Pop conforme sinalizado (\Cref{fig:quantitativo-CentroPop})
...
As situações de imigração / refúgio  vem crescendo no ambito das demandas do PAEFI, os dados do (\Cref{fig:paefi6_creas})sinaliza uma redução de 74% para 68% dos municípios que informam não atender essa demanda e, em sequencia, pode-se observar que todos os públicos aumentam ao longo destes dois anos.
...
No que se refere ao atendimento de violências e violações a povos e comunidades tradicionais o (\Cref{fig:creas-tradicionais}) destaca-se que dos seis povos e comundiades destacadas no grafico, em ambito nacional as maiores demandas estão para os públicos de comunidades quilombolas com 36%, povos indígenas com 35% e ribeirinhos com 29%. As regiões Sul e Centro Oeste possuem maior demanda de atendimento do público povos indígenas sendo respectivamente 72% e 59% respectivamente. A região Nordeste se destaca com atendimento de comundiades quilombolas com 55% e a Norte com 74% de demandas de comunidades ribeirinhas. 
...
>>>>>>> 906cd14f6803c6895797ee6f5f0d7bdc693e0f31
...
O (\Cref{fig:CREAS-MSE}) sinaliza que 89% dos CREAS ofertam esse serviços, esse número aumentou 8 pontos percentuais se comparado a ano de 2017.\footnote{Série só possível de ser selecionada historicametne a partir de 2017}
...
As ações e atividades no ambito do CREAS,\footnote{Série só possível de ser selecionada historicametne a partir de 2018} de acordo com  (\Cref{fig:creas-pse-domicilio}) a quantidade de Serviço de Proteção Social Especial para Pessoas com Deficiência e Pessoas Idosas com equipe específica para sua execução reduz em paralelo aumenta a oferta sem equipe exclusiva. Há redução também em relação aos municípios que deixam de ofertar este serviços que vai de 20% em 2018 para 31% em 2022. O (\Cref{fig:creas-pse-domicilio}) também destaca aumento dos municípios que realziam a oferta do Serviços, mas não no CREAS. 


```{r creas-pse-domicilio, fig.cap = "Percentual de CREAS que realizam Serviço de proteção social Especial para Pessoas com Deficiência, Idosas e suas Famílias, 2018 a 2022", fig.footer = "Fonte: MDS, Censo SUAS."}
...
Ao analisar a oferta de serviço especializado em abordagem social pelos municípios através do (\Cref{fig:paefi_creas}), no período de 2018 a 2022 houve uma redução da oferta deste serviço no ambito dos CREAS \footnote{Série só possível de ser selecionada historicametne a partir de 2018 com exeção do ano de 2020}. Destaca-se também a redução da oferta com serviços com e sem equipe exclusiva para este serviço.  
...
No território Nacional há 543 municípios que possuem este serviço conforme dados de 2022. Esse número cresceu em relação ao ano de 2017 conforme pode ser observado (\Cref{fig:fam_acolhedora}). O gráfico também referencia a oferta destes serviços por grandes regiões, as regiões Sul e Sudeste se destacam como o maior número quantitativo e a Região Nordeste em termos de crescimento proporcional neste período. AS regiões Centro Oeste e Norte foram as que menos tiveram crescimento da oferta do Serviço de Acolhimento em Família Acolhedora no ambito dos municípios.
