...
```{r estados-atualizacao-lei, fig.cap = "Quantidade de estados por ano de atualização da Lei Estadual de regulamentação do Sistema Único de Assistência Social (SUAS) - Brasil, 2022", fig.footer = "Fonte: MDS, Censo SUAS."}
...
grafico<-tabela %>% 
  mutate(Categoria = factor(Categoria, levels = levels(ordenado))) %>% 
  ggplot(aes(x = Categoria, y = n, group = Nível)) +
  geom_col(aes(fill = Nível), position = "dodge") +
  geom_text(aes(label = n), hjust = -0.5, position = position_dodge(width = 0.9)) +
  scale_y_continuous(expand = expand_scale(mult = c(0, .1))) +
  coord_flip() +
  theme(legend.position="bottom",
        legend.title = element_blank(),
