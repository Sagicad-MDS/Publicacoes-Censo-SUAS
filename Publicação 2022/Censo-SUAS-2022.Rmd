...
f_grafico_col_numero = function(df, x, y, grupo, legend_nrow){
  x <- enquo(x)
  y <- enquo(y)
  grupo <- enquo(grupo)
...
grafico<-tabela %>% 
  mutate(Categoria = factor(Categoria, levels = levels(ordenado))) %>% 
  f_grafico_col_numero_flip(Categoria, n, Nível)
...
#  bind_rows(f_quantitativo_regiao_ano(cras_2016, Região, "2016")) %>% 
  ...
  f_grafico_col_numero(Região, n, Ano, 1)
...
```{r cmas_se, fig.cap = "Percentual de Conselhos Estaduais que possuem secretárias/os executivas/os exclusivamente no Conselho"}
...
grafico<-tabela %>% 
  ungroup() %>% 
  mutate(Categoria = factor(Categoria, levels = levels(ordenado))) %>% 
  f_grafico_col_percent_flip(Categoria, Percentual, Nível, 1)
...
grafico <- f_grupo_media(idcras_2014, `IDCRAS 2014sintético`, "ID CRAS", "2014") %>%
  ...
  f_grafico_col_numero(Eixo_X, m, Grupo, 1)
...
grafico <- f_quantitativo_niveis(idcras_2022, `Estrutura Física`, "Estrutura Física", niveis_ID) %>% 
  ...
  mutate(Eixo_X = factor(Eixo_X, levels = c("Estrutura Física",
                                            "Recursos Humanos",
                                            "Serviços & Benefícios"))) %>% 
  f_grafico_col_numero(Eixo_X, n, Grupo, 1)
...
grafico <- f_grupo_media(idconselho_2014, IDConselho, "ID Conselho", "2014") %>%
  ...
  f_grafico_col_numero(Eixo_X, m, Grupo, 2)
