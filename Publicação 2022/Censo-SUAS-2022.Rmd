...
#ge_2019 <- read_excel("../Censo SUAS 2019/", sheet = 1)
...
gm_2020 <- read_excel("../Censo SUAS 2020/9 - Gestão Municipal/Censo_SUAS_2020_Gestão_Municipal_dados gerais.xlsx", sheet = 1)
...
cestrh_2019 <- read_excel("../Censo SUAS 2019/13 - CEAS/Censo_SUAS_2019_dados_gerais_RH_Conselho_Estadual_divulgação.xlsx", sheet = 1)", sheet = 1)
...
cmun_2022 <- read_excel("../Censo SUAS 2022/12 - CMAS/Censo_SUAS_2022_Conselho_Municipal_Dados_gerais.xls", sheet = 1)
...
unac_2018 <- read_excel("../Censo SUAS 2018/3 - Unidade de acolhimento/Censo_SUAS_2018_Acolhimento_M_DG_q_9.xlsx", sheet = 1)
...
f_acessibilidade_ABNT = function(df, q, cod_categoria){
  ...
  df %>%
    filter(!! q == "Sim, de acordo com a Norma da ABNT (NBR9050)" |
           ...
           !! q == "Sim, de acordo com a norma da ABNT" |
           !! q == "Sim, De acordo com a Norma da ABNT (NBR9050)" |
           # !! q == "SIM Mas não estão de acordo com a Norma da ABNT(NBR9050)" |
           # !! q == "Sim, mas não estão de acordo com a Norma da ABNT(NBR9050)" |
           # !! q == "Sim, mas não estão de acordo com a Norma da ABNT" |
           
             
           #coloquei acessibilidiade sem ser ABNT  
...
A estrutura administrativa na Política de Assistência Social e Constituição de areas essenciais são elementos importantes para gestão do SUAS. No ambito da gestão estadual, no que se refere a exclusividade da area de Assistência Social, percebe-se que nos úlimos 10 anos houve uma redução de areas exclusivas de assistencia social. Em 2012 existiam 25,9% \footnote{Estados: Amazonas, Acre, Pará, Amapá, Piauí, Paraíba, Sergipe e São Paulo} das secretarias estaduais exclusivas de Assistência Social, em 2022 esse número passa para 3,8%, \footnote{Estado do Amazonas} redução significativa de 87,2%.

Conforme a \Cref{fig:estados-caracteristica-secretaria}, 25 estados (96,4%) a Secretaria estadual estava associada a uma ou mais políticas setoriais, sendo predominante os grupos de duas e três politicas setoriais .

<!-- os dados históricos dos estados que reduziram area específica de assistencia social são bem significativos, assim vamos pensar grafico que mostre essa informação }. -->

```{r estados_sec_exc, fig.cap = "Percentual de Estados quanto a atualização do Plano Estadual de Assistência Social em 2022 - Brasil", fig.footer = "Fonte: MDS, Censo SUAS"}
...
estados_sec_exc  = function(df, q, ano){
  ...
  df %>%
    select(!! ano := !! q) %>% 
    gather("Categoria","Nível") %>% 
    mutate(Nível = case_when(Nível == "Não"~"Não",
                             TRUE ~ Nível)) %>% 
    filter(!is.na(Nível)) %>% 
...
grafico <- estados_sec_exc(ge_2022, q5, "2022") %>%
...
```{r estados_sec_exc, fig.cap = "Ações de busca ativa para GPTE (Grupos e Povos tradicionais específicos) - Brasil, 2020, 2021 e 2022", fig.footer = "Fonte: MDS, Censo SUAS.",eval=FALSE}

estados_sec_exc = function(df, q, ano, cod_categoria){
  q <- enquo(q)
  ano <- quo_name(ano)
  cod_categoria <- quo_name(cod_categoria)

#2022
  categoria <- case_when(!! cod_categoria == "1"~"Secretaria estadual exclusiva da área de Assistência Social",
                         !! cod_categoria == "2"~"Secretaria estadual em conjunto com outras políticas setoriais")
  ...
  df %>%
    ...
    summarise(n=n()) %>% 
    mutate(Percentual = n/sum(n)) %>% 
    filter(!! q == "Secretaria estadual exclusiva da área de Assistência Social") %>% 
    mutate("Categoria" = categoria, "Ano" = !! ano)
}

 tabela <- estados_sec_exc (ge_2012,q1,"2012","1") %>% 
  bind_rows(estados_sec_exc(ge_2012,q1,"2012","2")) %>% 
  #bind_rows(estados_sec_exc(posto_cadunico_2020,q21_3_1,"2020","3")) %>% 
  #bind_rows(estados_sec_exc(posto_cadunico_2020,q21_4_1,"2020","4")) %>% 
  #bind_rows(estados_sec_exc(posto_cadunico_2020,q21_5_1,"2020","5")) %>%
  #bind_rows(estados_sec_exc(posto_cadunico_2020,q21_6_1,"2020","6")) %>% 
  #bind_rows(estados_sec_exc(posto_cadunico_2020,q21_7_1,"2020","7")) %>%
  #bind_rows(estados_sec_exc(posto_cadunico_2020,q21_8_1,"2020","8")) %>% 
  #bind_rows(estados_sec_exc(posto_cadunico_2020,q21_9_1,"2020","9")) %>%
  #bind_rows(estados_sec_exc(posto_cadunico_2021,q21_1_1,"2021","1")) %>%
  #bind_rows(estados_sec_exc(posto_cadunico_2021,q21_2_1,"2021","2")) %>% 
  #bind_rows(estados_sec_exc(posto_cadunico_2021,q21_3_1,"2021","3")) %>% 
  #bind_rows(estados_sec_exc(posto_cadunico_2021,q21_4_1,"2021","4")) %>% 
  bind_rows(estados_sec_exc(ge_2022,q1,"2022","1")) %>%
  bind_rows(estados_sec_exc(ge_2022,q1,"2022","2")) %>% 
...
```{r estados-caracteristica-secretaria-2022, fig.cap = "Percentual de Secretarias Estaduais de acordo com composição administrativa relacionada a outras políticas - Brasil, 2022", fig.footer = "Fonte: MDS, Censo SUAS."}
...
No que se refere aos municípios, esse cenário nacional se configurou como contrário ao das gestões Estaduais. Observa-se um aumento das secretarias exclusivas de Assistência Social ao longo dos anos \footnote{a partir de 2019 essa pergunta foi retirada do formulário de gestão municipal}, com exceção da Região Nordeste (\Cref{fig:secretarias-municipais-exclusivas}).
...
Observa-se que meta chega em 2022 com 22 (85%) estados com as subdivisões administrativas das áreas de Proteção Social Básica, Proteção Social Especial e Gestão do SUAS constituídas formalmente. São 4 areas que não possuem 100% constituídas (formal e informal) são elas: Gestão do SUAS, Gestão do trabalho, Serviços e Beneficios e Regulação do SUAS. Esta última apesenta menor percentual de formalização, sendo 38,5% subdivisões constituídas formalmente conforme \Cref{fig:estados-constituicao-subdivisoes}.
...
A \Cref{fig:municipais-constituicao-subdivisoes} sinaliza que as areas do SUAS com maior percentual de constituição formal são respectivamente: Gestão do Cadastro Único e Bolsa Família, Gestão da Proteção Social Básica e Gestão do SUAS.  
...
Entratando, 11 estados que não possuem Lei do SUAS. Esse número estão presentes nas seguintes regiões do Brasil: 4 - Nordeste, 3 - Norte, 2 - Região Sul e 1 - Região Sudeste\footnote{ siglas dos Estado que não possuem Lei do SUAS - censo suas 2022: RR, PA, TO, PI, RN, SE, BA, SP, PR, SC e RS}.
...
# Censo 2022:
  mutate(Nível = case_when(Nível <= 2012~"Até 2012",
                           Nível <= 2016~"Entre 2013 e 2016",
                           Nível <= 2019~"Entre 2017 e 2019",
                           Nível <= 2022
                           
                           ~"Entre 2020 e 2022",
                           TRUE ~ as.character(Nível))) %>% 
  mutate(Nível = factor(Nível, levels = rev(c("Até 2012","Entre 2013 e 2016", "Entre 2017 e 2019", "Entre 2020 e 2022")))) %>% 
#-------------------
  group_by(Categoria, Nível) %>% 
  summarise(n=n()) %>% 
  group_by(Categoria) %>% 
...
No que se refere ao Planejamento, trata-se de uma obrigatoriedade para existência do Sistema Único e repasse de confinanciamento. O Plano Estadual de Assistência Social (PEAS) deve ser aprovado pelo Conselho Estadual de Assistência Social (CEAS). Nesta perspectiva, dados de 2022 sinalizam que 1 estado declarou não possui r Plano Estadual de Assistência Social, quando a atualização, 20% (5) dos estados atualizaram seu Plano no último ano conforme (\Cref{fig:PEAS}). \footnote{Para o ano de 2022 não houve a pergunta sobre Plano Municipal de Assistência}
...
O Plano de apoio técnico aos municípios também é um produto a ser realizado pelos Estados e previsto no último pacto de aprimoramento Estadual do SUAS\footnote{Resolução CNAS Nº2, de 16 de março de 2017}. De acordo com Censo Suas 2022, 57,69% dos estados possuem este plano pactuado na CIB. Este percentual reduziu em relação ao ano de 2017, na qual mais de 76% dos Estados informaram possui este documento pactuado neste instância de comissão intergestores bipartite. 




```{r plan_apoio_tec, fig.cap = "Percentual de Estados que possuiem Plano de Apoio Técnico pactuado na CIB - Brasil, 2013 - 2017 - 2022", fig.footer = "Fonte: MDS, Censo SUAS"}

plan_apoio  = function(df, q, ano){
  q <- enquo(q)
  ano <- quo_name(ano)
...
grafico <- plan_apoio(ge_2013, q85, "2013") %>%
  #bind_rows(plan_apoio(ge_2014, q26, "2014")) %>% 
  #bind_rows(plan_apoio(ge_2015, q19, "2015")) %>% 
  #bind_rows(plan_apoio(ge_2016, q27, "2016")) %>% 
  bind_rows(plan_apoio(ge_2017, q36, "2017")) %>%
  #bind_rows(plan_apoio(ge_2018, q_18, "2018")) %>%
  #bind_rows(plan_apoio(ge_, q18, "2019")) %>%
  #bind_rows(plan_apoio(ge_2020, q20, "2020")) %>%
  #bind_rows(plan_apoio(ge_2021, q18, "2021")) %>%
  bind_rows(plan_apoio(ge_2022, q20, "2022")) %>%
...
Abaixo (\Cref{fig:munic_vit_estadual}) o percentual de municípios do Brasil que informam sobre recebimento de visitas da gestão estadual para apoio técnico. 


```{r munic_vit_estadual, fig.cap = "Percentual de Municpioios que informam receber visitas de Apoio Técnico do Estado - Brasil, 2022", fig.footer = "Fonte: MDS, Censo SUAS"}

visit_estadual  = function(df, q, ano){
  q <- enquo(q)
  ano <- quo_name(ano)
...
grafico <- visit_estadual(gm_2022, q7_5, "2022") %>%
...
O modelo de gestão do SUAS preve cofinanciamento compartilhado entre os três entes federados.\footnote{Art. 30-A.  O cofinanciamento dos serviços, programas, projetos e benefícios eventuais, no que couber, e o aprimoramento da gestão da política de assistência social no Suas se efetuam por meio de transferências automáticas entre os fundos de assistência social e mediante alocação de recursos próprios nesses fundos nas 3 (três) esferas de governo.}. Nesta perspetiva, percebe-se um avanço nos últimos 10 anos sobre a participação dos estados no cofinanciamento estadual com participação de 96,15% (25) dos estados que informam cofinanciar os municípios (\Cref{fig:estados-cofinanciamento-municipios}). 
...
Em todas as regiões, o recebimento de recursos estaduais era majoritariamente fundo-a-fundo (\Cref{fig:municipios-recebimento-recursos}).
...
O ordenador de despesa responde pela emissão de empenho, autorização de pagamento dos recursos do fundo de assistencia social. Assim, recomenda-se que tenha amplo conhecimento sobre a política . O grafico abaixo (\Cref{fig:estado_ord_despesa}) o censo suas 2022 sinaliza que 73% dos gestores estaduais são ordenadores de dewspesa do fundo estadual. Dado que reduziu a partir de 2018.
...
No que se refere aos municípios, aproximadamente 80% dos secretários (as) municipais são ordenadores de despesa do Fundo Municipal. Esse percentual aumentou referente ao ano de 2018 (77,2%). A (\Cref{fig:munic_ord_despesa}) também mostra que os prefeitos (as) enquanto ordenadores de despesa da política de Assistência Social vem reduzindo ao longo dos anos. Dado que se revela como positivo, haja vista o gestor municipal encontra-se no cotidiano do planejamento desta política, sendo mais apropriado para tomar as decições sobre o orçamento.  



```{r munic_ord_despesa, fig.cap = "Percentual de municípios quanto a ordenação de despesa do fundo estadual - Brasil, 2013 - 2022", fig.footer = "Fonte: MDS, Censo SUAS."}
...
No que se refere a Educação Permanente, em relação aos Estados que possuem Núcleo de Educação Permante, a (\Cref{fig:NUEP}) evidencia que 88,5% dos estados possuem esta instancia colegiada. 


```{r NUEP, fig.cap = "Percentual de Estados que possuem Núcleo de Educação Permanete, 2022", fig.footer = "Fonte: MDS, Censo SUAS"}
...
Em relação ao Plano de Educação Permanente, em 2015, xx municípios indicaram ter Plano de Capacitação e Educação Permanente, o que equivale a 11,2% dos respondentes. Em 2022 o número de municípios que informou ter o Plano foi superior: xx municípios, o que representava 14,9% dos municípios respondentes.

Esse número varia conforme região sendo a região Nordeste com melhor indice: 26,6% dos municípios e a região sul com menor: 5,7% dos municípios conforme apresenta a (\Cref{fig:municipios-possuem-plano})
...
#  bind_rows(f_selecao_regiao_ano(gm_2021, q73, "Sim", Região, "2021")) %>%
  ...
  #não há esta pergunta para os estados2022_foiretirada
...
if(gera.graficos.office) grafico %>% graph2office(file="graficos", append = TRUE)
```
#colocar estados com plano de educação permanente
...
```{r conc_munic, fig.cap = "Percentual de municípios que realizaram concursos para trabalhadores nível superior ou médio do SUAS - Brasil, 2020 - 2021 - 2022", fig.footer = "Fonte: MDS, Censo SUAS"}

conc_munic  = function(df, q, ano){
  q <- enquo(q)
  ano <- quo_name(ano)
...
grafico <- conc_munic(gm_2021, q72_1_1, "2021") %>%
  #bind_rows(conc_munic(gm_2020, q72_1_1, "2020")) %>%
  #bind_rows(conc_munic(gm_2021, q72_1_1, "2021")) %>%
   bind_rows(conc_munic(gm_2022, q72_1_1, "2022")) %>%
  
  #2020 as informações esão 1 e 0, onde 1=sim e não=0
...
```{r conc_estado, fig.cap = "Percentual de estados que realizaram concursos para trabalhadores nível superior do SUAS - Brasil, 2022", fig.footer = "Fonte: MDS, Censo SUAS"}

conc_estado  = function(df, q, ano){
  q <- enquo(q)
  ano <- quo_name(ano)
...
grafico <- conc_estado(ge_2017, q16_1_1, "2017") %>%
   #bind_rows(conc_estado(ge_2017, q16_1_1, "2017")) %>%
   #bind_rows(conc_estado(ge_2018, q_36_1_1, "2018")) %>%
   #bind_rows(conc_estado(ge_2019, q36_1_1, "2019")) %>%
    bind_rows(conc_estado(ge_2020, q46_1_1, "2020")) %>%
   #bind_rows(conc_estado(ge_2021, q36_1_1, "2021")) %>%
   bind_rows(conc_estado(ge_2022, q46_1_1, "2022")) %>%
...
O Cadastro Único para Programas Sociais do Governo Federal estabelecido através do decreto nº 3.887/2001 tem a finalidade de coletar, sistematizar e disseminar informações que permitem a identificação e caracterização das condições socioeconômicas das famílias em situação de vulnerabilidade social, sobretudo para as famílias de baixa de renda. 

O objetivo é conhecer, incluir e aprimorar as políticas sociais, bem como potencializar acesso a Serviços, Programas, benefícios e Projetos. A gestão do Cadastro Único é compartilhada entre União, estados, municípios e Distrito Federal na qual cabe aos estados o apoio técnico, capacitação, monitoramento e avaliação. Nos municípios, encontram-se locais de cadastramento e toda gestão territorial da identificação das fmaílias neste cadastro público. 

As unidades do Cadastro Único podem ser encontradas em locais exclusivos ou na rede de atendimento socioassistencial de CRAS, CREAS, Centro Pop. Dados do Censo SUAS mostram uma evolução de locais de cadastramento. A partir de 2017 houve um aumento de 54% de postos de Cadastro Único, sobre a distribuição destes locais de Cadastro Único, destaca-se que em 2022, 31% (2.892) são unidades exclusivas de Cadastro Único e 68% estão em unidades da rede socioassistencial \footnote{CRAS, CREAS e CENTRO POP} conforme pode ser observado na tabela \ref{tab:qtd_equipamentos}.
...
A respeito dos Postos do Cadastro Único destacados acima, 61% encontram-se na sede da Secretaria de Assistência Social e 24,3% em postos exclusivos dos municípios. Os demais percentuais são encontrados em outra unidade administrativa como, em um serviço integrado, OSC´s, conselho ou escola.


No que se refere a gestão territorial, cabe as coordenações do Cadastro Único dos municípios atendimento, supervisão, monitoramento e avaliação dos processos de cadastramento, seja em postos específicos ou integradas as unidades da rede socioassistencial.

A produção de informações articuladas com Vigilância Socioassitencial também é uma ação importante para identificação das famílias vulnerabilizadas no território. Esses dados devem subsidiar busca ativa, ações de atualização, cadastral, bem como iniciativas de parcerias com outros orgãos. O gráfico  (\Cref{fig:acoes_cad}) sinaliza as ações e atividades desenvolvidas nos postos do cadastro único \footnote{estas informações são dos postos que executam exclusivamente atividades do cadastro único}.

 


```{r acoes_cad, fig.cap = "Ações desenvolvidas no ambito pelos postos do Cadastro Único - Brasil, 2020, 2021 e 2022", fig.footer = "Fonte: MDS, Censo SUAS."}
...
Faz parte de procedimentos de qualificação do Cadastro único ações de averiguação e revisão cadastral. A averiguação é um processo de verificação das informações registradas no Cadastro Único por meio da comparação dos dados declarados pelas famílias com outros dados e registros administrativos do governo federal. Já a revisão é um procedimento de atualização das famílias com registros desatualizados. O tempo considerado para um cadastro desatualizado é de 24 meses. O gráfico (\Cref{fig:ave_cad}) refere-se as informações de Averiguação e Revisão cadastral no ambito dos postos dos Cadastro Único dos municípios. Nota-se que ao longo dos anos, essas ações aumentaram, sobretudo as ações de busca ativa e identificação do público de averiguação e revisão cadastral como público prioritário.


```{r ave_cad, fig.cap = "Ações averiguação e revisão desenvolvidas pelos postos do Cadastro Único - Brasil, 2020, 2021 e 2022", fig.footer = "Fonte: MDS, Censo SUAS."}
...
#2020
  categoria <- case_when(!! cod_categoria == "1"~"Realiza busca ativa às famílias",
                         ...
                         !! cod_categoria == "3"~"Realiza multirões especificos para tender famílias",
...
```{r CadunicoGPTE, fig.cap = "Ações de busca ativa para GPTE (Grupos e Povos tradicionais específicos) - Brasil, 2020, 2021 e 2022", fig.footer = "Fonte: MDS, Censo SUAS.",eval=FALSE}

CadunicoGPTE= function(df, q, ano, cod_categoria){
  q <- enquo(q)
  ano <- quo_name(ano)
  cod_categoria <- quo_name(cod_categoria)

#2020
  categoria <- case_when(!! cod_categoria == "1"~"Povos indígenas",
                         !! cod_categoria == "2"~"Comunidade Quilombola",
                         !! cod_categoria == "3"~"Comunidade Ribeirinha",
                         !! cod_categoria == "4"~"Povos Ciganos",
                         !! cod_categoria == "5"~"Povos de Matriz Africana e de terreiro",
                         !! cod_categoria == "6"~"Extrativistas",
                         !! cod_categoria == "7"~"Pescadores artesanais",
                         !! cod_categoria == "8"~"Ribeirinhas",
                         !! cod_categoria == "9"~"Outros Grupos tradicionais e específicos")
 ...
 tabela <- CadunicoGPTE (posto_cadunico_2020,q21_1_1,"2020","1") %>% 
  bind_rows(CadunicoGPTE(posto_cadunico_2020,q21_2_1,"2020","2")) %>% 
  bind_rows(CadunicoGPTE(posto_cadunico_2020,q21_3_1,"2020","3")) %>% 
  bind_rows(CadunicoGPTE(posto_cadunico_2020,q21_4_1,"2020","4")) %>% 
  bind_rows(CadunicoGPTE(posto_cadunico_2020,q21_5_1,"2020","5")) %>%
  bind_rows(CadunicoGPTE(posto_cadunico_2020,q21_6_1,"2020","6")) %>% 
  bind_rows(CadunicoGPTE(posto_cadunico_2020,q21_7_1,"2020","7")) %>%
  bind_rows(CadunicoGPTE(posto_cadunico_2020,q21_8_1,"2020","8")) %>% 
  bind_rows(CadunicoGPTE(posto_cadunico_2020,q21_9_1,"2020","9")) %>%
  bind_rows(CadunicoGPTE(posto_cadunico_2021,q21_1_1,"2021","1")) %>%
  bind_rows(CadunicoGPTE(posto_cadunico_2021,q21_2_1,"2021","2")) %>% 
  bind_rows(CadunicoGPTE(posto_cadunico_2021,q21_3_1,"2021","3")) %>% 
  bind_rows(CadunicoGPTE(posto_cadunico_2021,q21_4_1,"2021","4")) %>% 
  bind_rows(CadunicoGPTE(posto_cadunico_2021,q21_5_1,"2021","5")) %>%
  bind_rows(CadunicoGPTE(posto_cadunico_2021,q21_6_1,"2021","6")) %>% 
  bind_rows(CadunicoGPTE(posto_cadunico_2021,q21_7_1,"2021","7")) %>%
  bind_rows(CadunicoGPTE(posto_cadunico_2021,q21_8_1,"2021","8")) %>% 
  bind_rows(CadunicoGPTE(posto_cadunico_2021,q21_9_1,"2021","9")) %>%
  bind_rows(CadunicoGPTE(posto_cadunico_2022,q21_1_1,"2022","1")) %>%
  bind_rows(CadunicoGPTE(posto_cadunico_2022,q21_2_1,"2022","2")) %>% 
  bind_rows(CadunicoGPTE(posto_cadunico_2022,q21_3_1,"2022","3")) %>% 
  bind_rows(CadunicoGPTE(posto_cadunico_2022,q21_4_1,"2022","4")) %>% 
  bind_rows(CadunicoGPTE(posto_cadunico_2022,q21_5_1,"2022","5")) %>%
  bind_rows(CadunicoGPTE(posto_cadunico_2022,q21_6_1,"2022","6")) %>% 
  bind_rows(CadunicoGPTE(posto_cadunico_2022,q21_7_1,"2022","7")) %>%
  bind_rows(CadunicoGPTE(posto_cadunico_2022,q21_8_1,"2022","8")) %>% 
  bind_rows(CadunicoGPTE(posto_cadunico_2022,q21_9_1,"2022","9")) %>%
   
   #os dados em branco não são contabilizados ... a partir de 2021 cresce muito as marcações em braco por isso a discrepancia nas informações. Avaliar a pertinencia, spobretudo pq 2021 e 2022 são referentes a periodos pandemicos.
...
A visita domiciliar é uma forma de coleta das informações, essa ação objetiva assegurar o acesso a inclusão ou atualização na residencia da família. De acordo com informações do (\Cref{fig:visit_dom}) as situações mais frequentes para visita domiciliar são para averiguação cadastral e inclusção/atualização de dados do BPC (Benefício de Prestação Continuada).
...
#2020
 ...
 tabela <- visit_dom (posto_cadunico_2020,q19_1,"2020","1") %>% 
  ...
  bind_rows(visit_dom(posto_cadunico_2021,q19_6,"2022","6")) %>% 
  bind_rows(visit_dom(posto_cadunico_2021,q19_99,"2022","7")) %>% 
...
Quanto a presença de estrutura física dos postos do Cadastro Único a acessibilidade \footnote{foram consideradas com ou sem ABNT} avançou nos últimos anos conforme ... 

<!-- o grafico só gerou 2022, mas as indicações estão em para os 3 últimos anos - verificar o erro -->

```{r postcad-acessibilidade, fig.cap = "Evolução percentual de postos do Cadastro Único segundo condições de acessibilidade de acordo com a Norma da ABNT - Brasil; 2020, 2021 e 2022", fig.footer = "Fonte: MDS, Censo SUAS."}
...
ordenado<-factor(ordenado, levels = unique(ordenado$Categoria))
...
```{r postcad-acessibilidade-situacao, fig.cap = "Percentual de Postos do Cadastro Único com existência de condições de acessibilidade de acordo com a Norma da ABNT, segundo situação do imóvel – Brasil, 2022", fig.footer = "Fonte: MDS, Censo SUAS."}
...
Em relação a computadores com acesso a internet, é indicador bem presente


```{r postcad-internet-percentual, fig.cap = "Evolução percentual de Cadastro Único com computadores com acesso à internet – Brasil;  2020, 2021 e 2022", fig.footer = "Fonte: MDS, Censo SUAS."}
...
no orgão gestor e profissionais específicos, destaca-se que   
...
coord_cadunico  = function(df, q, ano){
  q <- enquo(q)
  ano <- quo_name(ano)
...
grafico <- coord_cadunico(posto_cadunico_2020, q39_1, "2020") %>%
        bind_rows(coord_cadunico(posto_cadunico_2021, q39_1, "2021"))%>%
        bind_rows(coord_cadunico(posto_cadunico_2022, q39_1, "2022"))%>%
...
```{r equip-cadunico-cras, fig.cap = " Percentual de CRAS com locais para CadÚnico - 2017 - 2022", fig.footer = "Fonte: MDS, Censo SUAS",eval=FALSE}
...
```{r equip-cadunico-creas, fig.cap = " Percentual de CREAS com locais para CadÚnico e disposição da equipe - 2018 - 2022", fig.footer = "Fonte: MDS, Censo SUAS",eval=FALSE}
...
```{r equip-cadunico-centropop_, fig.cap = "Percentual de Centro Pop com locais para CadÚnico e disposição da equipe - 2018 - 2022", fig.footer = "Fonte: MDS, Censo SUAS",eval=FALSE}
...
grafico <- equip_cpop_cadunico(cpop_2018, q27, "2018") %>%
  ...
  #centro pop 2020 a base estar com códigos números - pedir a base nominal
...
```{r posto_cadunicoD, fig.cap = "Distribuição dos Postos do Cadastro Único - Brasil, 2020 a 2022", fig.footer = "Fonte: MDS, Censo SUAS.", eval=FALSE}
...
```{r atend-cadunico, fig.cap = " Percentual de postos do Cadastr Único - atendimento -2020 - 2021 - 2022", fig.footer = "Fonte: MDS, Censo SUAS",eval=FALSE}

atend  = function(df, q, ano){
  q <- enquo(q)
  ano <- quo_name(ano)
  ...
  df %>%
    select(!! ano := !! q) %>% 
    gather("Categoria","Nível") %>% 
    mutate(Nível = case_when(Nível == "A maioria das famílias é atendida no mesmo dia em que procuram esta unidade"~"A maioria das famílias é atendida no mesmo dia em que procuram esta unidade",
                             TRUE ~ Nível)) %>% 
    filter(!is.na(Nível)) %>% 
    mutate(Nível = factor(Nível, levels = c("A maioria das famílias é atendida no mesmo dia em que procuram esta unidade",
                                            "A minoria das famílias é atendida no mesmo dia em que procuram esta unidade",
                                            "Os atendimentos ocorrem apenas após agendamentos"))) %>% 
    group_by(Categoria, Nível) %>% 
    summarise(n=n()) %>% 
    group_by(Categoria) %>% 
    mutate(Percentual = n/sum(n), Posição = (cumsum(n/sum(n)) - 0.5 * n/sum(n)))
...
grafico <- atend(posto_cadunico_2020, q17, "2020") %>%
   bind_rows(atend(posto_cadunico_2021, q17, "2021")) %>% 
   bind_rows(atend(posto_cadunico_2022, q17, "2022")) %>% 
...
```{r atendimento_urgente, fig.cap = "Atendimento as demandas urgentes ás famílias nos Postos do Cadasatro Único - Brasil - 2020, 2020 e 2022", fig.footer = "Fonte: MDS, Censo SUAS."}
...
Desde 2014 o número de Centros de Convivência no Brasil aumentou, passando de 7.882 unidades em 2014 para 8.454 em 2016, em um acréscimo de 572 unidades. A região Norte tem a menor quantidade de unidades (238 ou 2,8% do total), seguida do Centro-Oeste com 568 Unidades (ou 6,7% do total). A região Sudeste tem o maior número de Centros de Convivência, com 4.035 Unidades (47,7% do total)
...
```{r CREAS-municipios-2, fig.cap = "Evolução de CREAS Regionais (res. 21/2023)", fig.footer = "Fonte: MDS, Censo SUAS", eval=FALSE }
...
<!-- i ideal seria colocar em número em vez de percentual -->

```{r creas-regionais, fig.cap = "Evolução de CREAS Regionais - por região", fig.footer = "Fonte: MDS, Censo SUAS."}
...
grafico <- f_selecao_regiao_ano (creas_2017, q2, "Regional", Região, "2017") %>%
   ...
   #não funcionou a função quantidade
...
```{r creas-acessibilidade, fig.cap = "Evolução percentual de CREAS segundo condições de acessibilidade de acordo com a Norma da ABNT - Brasil; 2012, 2017 e 2022", fig.footer = "Fonte: MDS, Censo SUAS."}
...
grafico <- f_maior0_regiao_ano(creas_2012, q14, Região, "2012") %>% 
   ...
   bind_rows(f_maior0_regiao_ano(creas_2022, q13, IBGE, "2022")) %>% 
...
<!-- Vadlson. GRAFICO ABAIXO ficou muito pequeno, será que conseguimos ampliar número -->
...
```{r cpop-acessibilidade-situacao, fig.cap = "Percentual de Centros POP com existência de condições de acessibilidade de acordo com a Norma da ABNT, segundo situação do imóvel – Brasil, 2022", fig.footer = "Fonte: MDS, Censo SUAS.",eval=FALSE}
...
```{r cdia-acessibilidade, fig.cap = "Evolução percentual dos Centros-Dia segundo condições de acessibilidade de acordo com a Norma da ABNT - Brasil, 2015 e 2022", fig.footer = "Fonte: MDS, Censo SUAS.",eval=FALSE}
...
```{r unac_acessibilidade, fig.cap = "Evolução percentual de Unidades de Acolhimento Municipais segundo condições de acessibilidade de acordo com a Norma da ABNT - Brasil; 2017 e 2022", fig.footer = "Fonte: MDS, Censo SUAS,",eval=FALSE}
...
A participação social é uma das diretrizes estabelecidas pela Constituição Federal de 1988 para a organização das ações da Assistência Social. Nesse sentido, a Lei Orgânica da Assistência Social (LOAS) \footnote{Lei nº 8.742, de 7 de dezembro de 1993: Dispõe sobre a organização da Assistência Social e dá outras providências. (\url{http://www.planalto.gov.br/ccivil_03/Leis/L8742compilado.htm})}, que dispõe sobre a sua organização, instituiu em seu artigo 16 os Conselhos de Assistência Social em âmbito nacional, estadual e municipal como instâncias de deliberação colegiada do SUAS, cuja composição deve ser paritária entre governo e sociedade civil. 

Os Conselhos integram o Sistema Único de Assistência Social (SUAS), juntamente com o governo e as entidades e organizações de assistência social. A Resolução do Conselho Nacional de Assistência Social (CNAS) nº 237/2006, \footnote{(\url{https://www.mpba.mp.br/sites/default/files/biblioteca/crianca-e-adolescente/suas/resolucoes/resolucao_n_237_2006.pdf})} estabelece em seu artigo 3º as competências dos Conselhos de Assistência Social, das quais se destacam: a aprovação da respectiva Política de Assistência Social, em consonância com a Política Nacional de Assistência Social (PNAS); a normatização e regulação de ações e prestação de serviços, em conjunto com a gestão; o acompanhamento, avaliação e fiscalização da gestão de recursos; a inscrição e a fiscalização das entidades e organizações da assistência social; e a aprovação da proposta orçamentária dos recursos da assistência social e dos critérios de partilha dos recursos, dentre outras atribuições.

A mesma Resolução estabelece que na composição dos Conselhos deve haver, além de representantes do governo, representantes da sociedade civil, incluindo usuários, entidades e organizações de assistência social e entidades de trabalhadores do setor.
...
No que se refere aos dados do censo suas, o percentual de municípios que respondem ao formulário de Conselho Municipal de Assistência Social é acima de 90%, com algumas variações entre 2012 e 2022. É importante pontuar que as variações no período analisado não significam necessariamente redução no número de municípios com conselhos, uma vez que podem ocorrer variações no número de conselhos municipais que respondem ao Censo SUAS.

Abaixo o gráfico tras informações da quantidade de conselhos municipais, por região, que responderam ao Censo SUAS.


```{r quantitativo-CMAS, fig.cap = "Evolução do quantitativo de Conselhos Municipais, segundo grandes regiões; 2012 - 2022", fig.footer = "Fonte: MDS, Censo SUAS.",eval=FALSE}

grafico <- f_quantitativo_regiao_ano (cmun_2012, Região, "2012") %>% 
  #bind_rows(f_quantitativo_regiao_ano (cmun_2010, Região, "2010")) %>%
  #bind_rows(f_quantitativo_regiao_ano(cmun_2011, Região, "2011")) %>% 
  #bind_rows(f_quantitativo_regiao_ano(cmun_2012, Região, "2012")) %>% 
  #bind_rows(f_quantitativo_regiao_ano(cmun_2013, Região, "2013")) %>% 
  #bind_rows(f_quantitativo_regiao_ano(cmun_2014, IBGE, "2014")) %>% 
  #bind_rows(f_quantitativo_regiao_ano(cmun_2015, Região, "2015")) %>% 
  #bind_rows(f_quantitativo_regiao_ano(cmun_2016, Região, "2016")) %>% 
  #bind_rows(f_quantitativo_regiao_ano(cmun_2017, Região, "2017")) %>% 
  #bind_rows(f_quantitativo_regiao_ano(cmun_2018, Região, "2018")) %>% 
  #bind_rows(f_quantitativo_regiao_ano(cmun, Região, "2019")) %>% 
  #bind_rows(f_quantitativo_regiao_ano(cmun_2020, Região, "2020")) %>% 
...
```{r quantitativo-CEAS, fig.cap = "Evolução do quantitativo de Conselhos Estaduais, segundo grandes regiões; 2012 - 2022", fig.footer = "Fonte: MDS, Censo SUAS.",eval=FALSE}
...
A sede e local especifico para funcionamento dos Coselhos é uma demanda 



```{r Cmas_sede, fig.cap = "Percentual de Conselhos municipais que possuem local/sede específico para funcionamento - Brasil, 2012 - 2017 - 2022"}

cmas_sede  = function(df, q, ano){
  q <- enquo(q)
  ano <- quo_name(ano)
...
grafico <- cmas_sede(cmun_2012, q8, "2012") %>%
   #bind_rows(cmas_sede(cmun_2013, q8, "2013")) %>%
   bind_rows(cmas_sede(cmun_2014, q9, "2014")) %>%
   bind_rows(cmas_sede(cmun_2015, q9, "2015")) %>%
   bind_rows(cmas_sede(cmun_2016, q6, "2016")) %>%
   bind_rows(cmas_sede(cmun_2017, q7, "2017")) %>%
   bind_rows(cmas_sede(cmun_2018, q_6, "2018")) %>%
   bind_rows(cmas_sede(cmun_2019, q6, "2019")) %>%
   bind_rows(cmas_sede(cmun_2020, q6, "2020")) %>%
   bind_rows(cmas_sede(cmun_2021, q6, "2021")) %>%
   bind_rows(cmas_sede(cmun_2022, q6, "2022")) %>%
...
```{r Ceas_sede, fig.cap = "Percentual de Conselhos Estaduais que possuem local/sede específico para funcionamento - Brasil, 2012 - 2022"}

ceas_sede  = function(df, q, ano){
  q <- enquo(q)
  ano <- quo_name(ano)
...
grafico <- ceas_sede(cest_2012, q8, "2012") %>%
   #bind_rows(ceas_sede(cest_2013, q8, "2013")) %>%
   #bind_rows(ceas_sede(cest_2014, q9, "2014")) %>%
    bind_rows(ceas_sede(cest_2015, q8, "2015")) %>%
    bind_rows(ceas_sede(cest_2016, q6, "2016")) %>%
    bind_rows(ceas_sede(cest_2017, q7, "2017")) %>%
    #bind_rows(ceas_sede(cest_2018, q_6, "2018")) %>%
    bind_rows(ceas_sede(cest_2019, q6, "2019")) %>%
    bind_rows(ceas_sede(cest_2020, q6, "2020")) %>%
    bind_rows(ceas_sede(cest_2021, q6, "2021")) %>%
   bind_rows(ceas_sede(cest_2022, q6, "2022")) %>%
  
  #2013 AS QUESTÕES ESÃO COM AS RESPOSTAS "";
  #2014 incluiu "Não sabe"
...
```{r Cmas_se, fig.cap = "Percentual de Conselhos municipais com secretarias executivas exclusivas - Brasil, 2012 - 2017 - 2022"}
...
```{r Ceas_sec, fig.cap = "Percentual de Conselhos Estaduais que possuem secretaria executiva - Brasil, 2012 - 2017 - 2022"}

ceas_se  = function(df, q, ano){
  q <- enquo(q)
  ano <- quo_name(ano)
...
grafico <- ceas_se(cest_2012, q8, "2012") %>%
  #bind_rows(ceas_se(cest_2013, q8, "2013")) %>%
  #bind_rows(ceas_se(cest_2014, q8, "2014")) %>%
  #bind_rows(ceas_se(cest_2015, q8, "2015")) %>%
  #bind_rows(ceas_se(cest_2016, q8, "2016")) %>%
   bind_rows(ceas_se(cest_2017, q7, "2017")) %>%
  #bind_rows(ceas_se(cest_2018, q8, "2018")) %>%
  #bind_rows(ceas_se(cest_2019, "2019")) %>%
  #bind_rows(ceas_se(cest_2020, q8, "2018")) %>%
  #bind_rows(ceas_se(cest_2021, q8, "2018")) %>%
   bind_rows(ceas_se(cest_2022, q6, "2022")) %>%
...
<!-- veriricar a função para rodar todas os Niveis  -->

```{r Qtd_reuniao_cmas, fig.cap = "Quantidade de reuniões realizadas nos conselhos municipais - Brasil, 2022", fig.footer = "Fonte: MDS, Censo SUAS."}


tabela<-cmun_2022 %>%
#-------------------
# Questionários 2022:
  select("Quantidade de reuniões - ordinárias e extraordinárias"=q25) %>% 
#-------------------
  gather("Categoria","Nível") %>% 
  mutate(Categoria=str_wrap(Categoria,width=22)) %>% 
...
#-------------------
# Censo 2022:
  mutate(Nível = case_when(Nível <= 5~"Até 5",
                           Nível <= 5~"Entre 5 e 8",
                           Nível <= 8~"Entre 8 e 12",
                           Nível <= 12~"Acima 12",
                           TRUE ~ as.character(Nível))) %>% 
  mutate(Nível = factor(Nível, levels = rev(c("Até 5","Entre 5 e 8", "Entre 8 e 12", "acima de 12")))) %>% 
#-------------------
  group_by(Categoria, Nível) %>% 
  summarise(n=n()) %>% 
  group_by(Categoria) %>% 
...
```{r IGD_CMAS, fig.cap = "Percentual de conselhos municipais na qual são destinados pelo menos 3% dos IGB Bolsa Família e SUAS", fig.footer = "Fonte: MDS, Censo SUAS."}

cmas_igd  = function(df, q, ano){
  q <- enquo(q)
  ano <- quo_name(ano)
...
grafico <- cmas_igd(cmun_2012, q7, "2012") %>%
   bind_rows(cmas_igd(cmun_2013, q7, "2013")) %>%
   bind_rows(cmas_igd(cmun_2014, q8, "2014")) %>%
   bind_rows(cmas_igd(cmun_2015, q8, "2015")) %>%
   bind_rows(cmas_igd(cmun_2016, q5, "2016")) %>%
   bind_rows(cmas_igd(cmun_2017, q18, "2017")) %>%
   bind_rows(cmas_igd(cmun_2018, q_17, "2018")) %>%
   bind_rows(cmas_igd(cmun_2019, q17, "2019")) %>%
   bind_rows(cmas_igd(cmun_2020, q20, "2020")) %>%
   bind_rows(cmas_igd(cmun_2021, q20, "2021")) %>%
   bind_rows(cmas_igd(cmun_2022, q20, "2022")) %>%
...
```{r IGD_CEAS, fig.cap = "Quantidade de conselhos estaduais na qual são destinados pelo menos 3% dos IGB Bolsa Família e SUAS", fig.footer = "Fonte: MDS, Censo SUAS."}

ceas_igd  = function(df, q, ano){
  q <- enquo(q)
  ano <- quo_name(ano)
...
grafico <- ceas_igd(cest_2012, q7, "2012") %>%
   bind_rows(ceas_igd(cest_2013, q7, "2013")) %>%
   bind_rows(ceas_igd(cest_2014, q8, "2014")) %>%
   bind_rows(ceas_igd(cest_2015, q8, "2015")) %>%
   bind_rows(ceas_igd(cest_2016, q5, "2016")) %>%
   bind_rows(ceas_igd(cest_2017, q18, "2017")) %>%
   bind_rows(ceas_igd(cest_2018, q_17, "2018")) %>%
   bind_rows(ceas_igd(cest_2019, q17, "2019")) %>%
   bind_rows(ceas_igd(cest_2020, q20, "2020")) %>%
   bind_rows(ceas_igd(cest_2021, q20, "2021")) %>%
   bind_rows(ceas_igd(cest_2022, q20, "2022")) %>%
...
```{r Cmas_plan, fig.cap = "Percentual de Conselhos Municipais que possuem e que deliberaram sobre o Plano de Assistência Social - Brasil, 2012 - 2017 - 2022"}

cmas_pmas  = function(df, q, ano){
  q <- enquo(q)
  ano <- quo_name(ano)
...
grafico <- cmas_pmas(cmun_2012, q35_1, "2012") %>%
  bind_rows(cmas_pmas(cmun_2013, q28, "2013")) %>%
  bind_rows(cmas_pmas(cmun_2014, q28, "2014")) %>%
  bind_rows(cmas_pmas(cmun_2015, q28, "2015")) %>%
  bind_rows(cmas_pmas(cmun_2016, q21, "2016")) %>%
  bind_rows(cmas_pmas(cmun_2017, q38, "2017")) %>%
  bind_rows(cmas_pmas(cmun_2018, q_34_4, "2018")) %>%
  #bind_rows(cmas_pmas(cmun_, q, "2019")) %>%
  bind_rows(cmas_pmas(cmun_2020, q33_4, "2020")) %>%
  bind_rows(cmas_pmas(cmun_2021, q33_4, "2021")) %>%
  bind_rows(cmas_pmas(cmun_2022, q33_4, "2022")) %>%
...
```{r Ceas_se, fig.cap = "Percentual de Conselhos Estaduais que possuem e que deliberaram sobre o Plano de Assistência Social - Brasil, 2012 - 2017 - 2022"}

ceas_pmas  = function(df, q, ano){
  q <- enquo(q)
  ano <- quo_name(ano)
...
grafico <- ceas_se(cest_2012, q35_1, "2012") %>%
  #bind_rows(ceas_se(cest_2013, q8, "2013")) %>%
  #bind_rows(ceas_se(cest_2014, q8, "2014")) %>%
  #bind_rows(ceas_se(cest_2015, q8, "2015")) %>%
  #bind_rows(ceas_se(cest_2016, q38, "2016")) %>%
   bind_rows(ceas_se(cest_2017, q38, "2017")) %>%
  #bind_rows(ceas_se(cest_2018, q8, "2018")) %>%
  #bind_rows(ceas_se(cest_2019, "2019")) %>%
  #bind_rows(ceas_se(cest_2020, q8, "2018")) %>%
  #bind_rows(ceas_se(cest_2021, q8, "2018")) %>%
   bind_rows(ceas_se(cest_2022, q33_4, "2022")) %>%
...
```{r Cmas_be, fig.cap = "Percentual de Conselhos Municipais que deliberam sobre critérios de Benefícios Eventuais - Brasil, 2012 - 2017 - 2022"}
...
grafico <- cmas_be(cmun_2012, q38, "2012") %>%
  #bind_rows(cmas_be(cmun_2013, q, "2013")) %>%
  ...
  #bind_rows(cmas_be(cmun_2019, q37, "2019")) %>%
  bind_rows(cmas_be(cmun_2020, q37, "2020")) %>%
...
```{r Ceas_be, fig.cap = "Percentual de Conselhos Estaduais que deliberam sobre critérios de Benefícios Eventuais - Brasil, 2012 - 2017 - 2022"}
...
grafico <- ceas_be(cest_2012, q38, "2012") %>%
  #bind_rows(ceas_be(cest_2013, q8, "2013")) %>%
  #bind_rows(ceas_be(cest_2014, q8, "2014")) %>%
  #bind_rows(ceas_be(cest_2015, q8, "2015")) %>%
  #bind_rows(ceas_be(cest_2016, q8, "2016")) %>%
   bind_rows(ceas_be(cest_2017, q44, "2017")) %>%
  #bind_rows(ceas_be(cest_2018, q8, "2018")) %>%
  #bind_rows(ceas_be(cest_2019, "2019")) %>%
  #bind_rows(ceas_be(cest_2020, q8, "2018")) %>%
  #bind_rows(ceas_be(cest_2021, q8, "2018")) %>%
...
```{r Cmas_ppa, fig.cap = "Percentual de Conselhos Municipais que deliberam sobre a proposta anual do orçamento do executivo - Brasil, 2012 - 2017 - 2022"}

Cmas_ppa  = function(df, q, ano){
  q <- enquo(q)
  ano <- quo_name(ano)
...
grafico <- Cmas_ppa(cmun_2012, q38, "2012") %>%
  #bind_rows(Cmas_ppa(cmun_2013, q, "2013")) %>%
  bind_rows(Cmas_ppa(cmun_2014, q31, "2014")) %>%
  bind_rows(Cmas_ppa(cmun_2015, q31, "2015")) %>%
  bind_rows(Cmas_ppa(cmun_2016, q24, "2016")) %>%
  bind_rows(Cmas_ppa(cmun_2017, q42, "2017")) %>%
  bind_rows(Cmas_ppa(cmun_2018, q_37, "2018")) %>%
  #bind_rows(Cmas_ppa(cmun_2019, q37, "2019")) %>%
  bind_rows(Cmas_ppa(cmun_2020, q34, "2020")) %>%
  bind_rows(Cmas_ppa(cmun_2021, q34, "2021")) %>%
  bind_rows(Cmas_ppa(cmun_2022, q34, "2022")) %>%
...
```{r Ceas_ppa, fig.cap = "Percentual de Conselhos Estaduais que deliberam sobre a proposta anual do orçamento do executivo - Brasil, 2012 - 2017 - 2022"}

Ceas_ppa  = function(df, q, ano){
  q <- enquo(q)
  ano <- quo_name(ano)
...
grafico <- Ceas_ppa(cest_2012, q29, "2012") %>%
  #bind_rows(Ceas_ppa(cest_2013, q, "2013")) %>%
  #bind_rows(Ceas_ppa(cest_2014, q40, "2014")) %>%
  #bind_rows(Ceas_ppa(cest_2015, q38, "2015")) %>%
  #bind_rows(Ceas_ppa(cest_2016, q31, "2016")) %>%
  #bind_rows(Ceas_ppa(cest_2017, q44, "2017")) %>%
  #bind_rows(Ceas_ppa(cest_2018, q_39, "2018")) %>%
  #bind_rows(Ceas_ppa(cest_2019, q37, "2019")) %>%
  #bind_rows(Ceas_ppa(cest_2020, q37, "2020")) %>%
  #bind_rows(Ceas_ppa(cest_2021, q37, "2021")) %>%
   bind_rows(Ceas_ppa(cest_2022, q34, "2022")) %>%
...
## ID CRAS - Indíce de Desenvolvimento do CRAS

```{r ID-CRAS, fig.cap = "Evolução das médias do ID CRAS, 2014 a 2022", fig.footer = "Fonte: MDS, Censo SUAS."}
...
```{r ID-CRAS-nivel, fig.cap = "Quantidade de CRAS em cada nível de ID, 2022", fig.footer = "Fonte: MDS, Censo SUAS."}
...
## ID CREAS -Indíce de Desenvolvimento do CREAS

```{r ID-CREAS, fig.cap = "Evolução das médias do ID CREAS, 2014 a 2022", fig.footer = "Fonte: MDS, Censo SUAS."}
...
```{r ID-CREAS-nivel, fig.cap = "Quantidade de CREAS em cada nível de ID, 2022", fig.footer = "Fonte: MDS, Censo SUAS."}
...
## ID CONSELHO - Indíce de Desenvolvimento dos Conselhos Municipais de Assistência Social

```{r ID-conselho, fig.cap = "Evolução das médias do ID Conselho, 2014 a 2022", fig.footer = "Fonte: MDS, Censo SUAS."}
...
```{r ID-conselho-nivel, fig.cap = "Quantidade de Conselhos em cada nível de ID, 2022", fig.footer = "Fonte: MDS, Censo SUAS."}
...
<!-- consegui gerar o gráfico, para os anos de 2019/2020 e 2021 os conselhos que Não preencheu o Censo SUAS Conselho 2019, na celula sai a seguinte informação " -- ", assim temos que ver uma possibilidade de não contabilizar para poder gerar -->

<!-- não consegui gerar ID Sintético, porque não são números inteiros, depois vou tentar ver média variavel que possa contabilizar -->


```{r id-conselho-nacional, fig.cap = "Evolução do ID Conselho - Dimensão Estrutura Administrativa - 2014 - 2022", fig.footer = "Fonte: MDS, Censo SUAS."}
...
Estrutura Administrativa dos conselhos municipais piorou, sobretudo após 2016. 

```{r id-conselho-estrutura, fig.cap = "Evolução do ID Conselho - Dimensão Estrutura Administrativa - 2014 - 2022", fig.footer = "Fonte: MDS, Censo SUAS."}
...
```{r id-conselho-dinamica, fig.cap = "Evolução do ID Conselho - Dimensão dinâmica de Funcionamento - 2014 - 2022", fig.footer = "Fonte: MDS, Censo SUAS."}
...
```{r id-conselho-composicao, fig.cap = "Evolução do ID Conselho - Composição do Conselho - 2014 - 2022", fig.footer = "Fonte: MDS, Censo SUAS."}
