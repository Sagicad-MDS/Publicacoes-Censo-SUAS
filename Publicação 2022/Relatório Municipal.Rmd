...
```{r pacotes, include=FALSE, cache=params$pacotes_carregados, fig.path='Relatório-Municipal_files/pdf/'}
...
porte_creas <- idcreas_2022  %>%
                filter(`Código IBGE` == municipio)  %>%
                select(Porte)
porte_conselho <- idconselho_2022  %>%
                filter(IBGE7 == municipio)  %>%
                select(Porte)
...
grafico <- f_quantitativo_niveis_filtro(idcras_2022, `Código IBGE`, municipio, `Estrutura Física`, "Estrutura Física", niveis_ID) %>% 
  bind_rows(f_quantitativo_niveis_filtro(idcras_2022, `Código IBGE`, municipio, `Recursos Humanos`, "Recursos Humanos", niveis_ID)) %>%
  bind_rows(f_quantitativo_niveis_filtro(idcras_2022, `Código IBGE`, municipio, `Serviços `, "Serviços & Benefícios", niveis_ID)) %>% 
...
tabela <- unidades_municipio %>%
            tabyl(q1) %>%
            select(q1, n) %>%
            kable(caption=sprintf("Quantidade de %s em 2022 no município de %s por capacidade de referenciamento.", equipamento, nome_municipio),
...
grafico <- f_quantitativo_niveis_filtro(idcreas_2022, `Código IBGE`, municipio, `Estrutura Física`, "Estrutura Física", niveis_ID) %>% 
  bind_rows(f_quantitativo_niveis_filtro(idcreas_2022, `Código IBGE`, municipio, `Recursos Humanos`, "Recursos Humanos", niveis_ID)) %>%
  bind_rows(f_quantitativo_niveis_filtro(idcreas_2022, `Código IBGE`, municipio, `Serviços `, "Serviços & Benefícios", niveis_ID)) %>% 
...
cat("A \\Cref{fig:ID-conselho} mostra a evolução das médias do ID Conselho ao longo dos anos no município de ", nome_municipio, ", ", uf_municipio, ".", sep="")
```

```{r ID-conselho, fig.cap = sprintf(" Evolução das médias do ID Conselho, 2014 a 2022: %s, %s", nome_municipio, uf_municipio), fig.footer = "Fonte: MDS, Censo SUAS.", eval=(existe_equipamento>0)}
...
```{r}
unidades_municipio <- idconselho_2022 %>%
                       filter(IBGE7 == municipio)
