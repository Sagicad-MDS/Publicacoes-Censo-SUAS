---
title: Relatório Municipal de Indicadores 2022
date:
...
# municipio: 110012 # CRAS Serviços nível 1 município Grand
---
...
```{r pacotes, include=FALSE, cache=TRUE, fig.path='Relatório-Municipal_files/pdf/'}
...
```{r carregamento, cache=TRUE, fig.path='Relatório-Municipal_files/pdf/'}
...
porte_cras <- idcras_2022  %>%
                filter(`Código IBGE` == municipio)  %>%
                select(Porte)
porte_creas <- idcreas_2022  %>%
                filter(`Código IBGE` == municipio)  %>%
                select(Porte)
...
A \Cref{fig:ID-CRAS} mostra a evolução das médias do ID CRAS ao longo dos anos no município de `r nome_municipio`, `r uf_municipio`.
...
```{r}
unidades_municipio <- idcras_2022 %>%
                       filter(`Código IBGE` == municipio)

n_unidades_municipio <- nrow(unidades_municipio)
```

```{r ID-CRAS-nivel, fig.cap = sprintf("Quantidade de CRAS em cada nível de ID, 2022: %s, %s", nome_municipio, uf_municipio), fig.footer = "Fonte: MDS, Censo SUAS.", eval=n_unidades_municipio>1}

grafico <- f_quantitativo_filtro(idcras_2022, `Código IBGE`, municipio, `Estrutura Física`, "Estrutura Física") %>% 
  bind_rows(f_quantitativo_filtro(idcras_2022, `Código IBGE`, municipio, `Recursos Humanos`, "Recursos Humanos")) %>%   bind_rows(f_quantitativo_filtro(idcras_2022, `Código IBGE`, municipio, `Serviços `, "Serviços & Benefícios")) %>% 
...
```{r cras-por-capacidade, fig.align='center'}
...
tabela <- unidades_municipio %>%
            tabyl(q1) %>%
            select(q1, n) %>%
            kable(caption=sprintf("Quantidade de %s no município de %s por capacidade de referenciamento.", equipamento, nome_municipio),
...
```{r eval=!n_unidades, results='asis'}
...
* Possuir **recepção**; e
* Possuir, no mínimo, **3 salas de atendimento**, sendo **pelo menos 1 com capacidade para 15 ou mais pessoas**; e
* Possuir **acessibiliadade, ao menos parcial**.
```
...
* Possuir no mínimo **1 Assistente Social** (2, se EV) e **1 Psicólogo**; e 
...
* Possuir no mínimo **2 Assistentes Sociais** (3, se EV) e **1 Psicólogo**; e 
* Possuir no mínimo **1** profissional de nível superior com **vínculo estatutário ou empregado público celetista**.
```
...
cat("A \\Cref{fig:ID-CREAS} mostra a evolução das médias do ID CREAS ao longo dos anos no município de ", nome_municipio, ", ", uf_municipio, ".", sep="")
```

```{r ID-CREAS, fig.cap = sprintf(" Evolução das médias do ID CREAS, 2014 a 2022: %s, %s", nome_municipio, uf_municipio), fig.footer = "Fonte: MDS, Censo SUAS.", eval=(existe_equipamento>0)}
...
```{r}
unidades_municipio <- idcreas_2022 %>%
                       filter(`Código IBGE` == municipio)

n_unidades_municipio <- nrow(unidades_municipio)
```

```{r ID-CREAS-nivel, fig.cap = sprintf("Quantidade de CREAS em cada nível de ID, 2022: %s, %s", nome_municipio, uf_municipio), fig.footer = "Fonte: MDS, Censo SUAS.", eval=n_unidades_municipio>1}

grafico <- f_quantitativo_filtro(idcreas_2022, `Código IBGE`, municipio, `Estrutura Física`, "Estrutura Física") %>% 
  bind_rows(f_quantitativo_filtro(idcreas_2022, `Código IBGE`, municipio, `Recursos Humanos`, "Recursos Humanos")) %>%   bind_rows(f_quantitativo_filtro(idcreas_2022, `Código IBGE`, municipio, `Serviços `, "Serviços & Benefícios")) %>% 
...
* Possuir kit equipamento: Telefone, Impressora, 2 ou mais computadores com Internet e Veículo próprio ou compartilhado; e
* Possuir acessibilidade (com ou sem ABNT); e
```
```{asis, echo=(n_unidades>0 && menor_nivel<5 && maior_nivel>3 && factor_porte<"Grande")}

* Possuir 3 salas ou mais para atendimento (5 se CREAS Regional), sendo pelo menos 1 com capacidade para 15 ou mais pessoas.
```
```{asis, echo=(n_unidades>0 && menor_nivel<5 && maior_nivel>3 && factor_porte>"Médio")}

* Possuir 5 salas ou mais para atendimento, sendo pelo menos 1 com capacidade para 15 ou mais pessoas.
```
...
A \Cref{fig:ID-conselho} mostra a evolução das médias do ID Conselho ao longo dos anos no município de `r nome_municipio`, `r uf_municipio`.
